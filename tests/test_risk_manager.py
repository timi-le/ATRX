...
from core.interfaces.trading_interfaces import Order, OrderSide, OrderType, Position
...
def sample_config():
    ...
    config_data = {
        ...
        "alerts": {
            "channels": {"log": True, "console": True, "pubsub": True},
            "levels": {
                "info": True,
                "warning": True,
                "critical": True,
...
def risk_manager(sample_config):
    ...
    manager = CoreRiskManager(
        config=config, initial_capital=100000.0, publisher=None, logger=Mock()
    )
    yield manager
...
class TestPreTradeRiskChecks:
    ...
    @pytest.mark.asyncio
    async def test_emergency_mode_rejection(
        self, risk_manager, sample_order, sample_positions
    ):
    ...
    @pytest.mark.asyncio
    async def test_trading_halt_rejection(
        self, risk_manager, sample_order, sample_positions
    ):
    ...
    async def test_max_positions_limit(self, risk_manager, sample_order):
        ...
        for i in range(8):  # max_concurrent_positions = 8
            positions[f"PAIR{i}"] = Position(
                symbol=f"PAIR{i}", quantity=1000, avg_price=1.0, unrealized_pnl=0.0
            )
    ...
    @pytest.mark.asyncio
    async def test_valid_trade_approval(
        self, risk_manager, sample_order, sample_positions
    ):
...
class TestPositionLimits:
    ...
    @pytest.mark.asyncio
    async def test_check_position_limits_symbol_exceeded(
        self, risk_manager, sample_positions
    ):
...
class TestEmergencyControls:
    ...
    async def test_emergency_mode_reset_success(self, risk_manager):
        ...
        with patch.object(
            risk_manager, "_check_recovery_conditions", return_value=True
        ):
            # Trigger emergency mode
            await risk_manager.emergency_stop("Test emergency")
            assert risk_manager.is_emergency_mode
...
class TestPnLTracking:
    ...
    async def test_rapid_loss_detection(self, risk_manager):
        ...
        rapid_loss_alerts = [
            alert
            for alert in risk_manager.active_alerts.values()
...
class TestAlertSystem:
    ...
    async def test_send_alert(self, risk_manager):
        ...
        await risk_manager._send_alert(
            RiskLevel.WARNING, "Test alert message", "test_metric", 0.05, 0.03
        )
    ...
    async def test_alert_frequency_limiting(self, risk_manager):
        ...
        # Send first alert
        await risk_manager._send_alert(
            RiskLevel.WARNING, "Test alert 1", "test_metric", 0.05, 0.03
        )

        # Send duplicate alert immediately (should be blocked)
        await risk_manager._send_alert(
            RiskLevel.WARNING, "Test alert 2", "test_metric", 0.06, 0.03
        )
    ...
    async def test_emergency_alert_frequency(self, risk_manager):
        ...
        await risk_manager._send_alert(
            RiskLevel.EMERGENCY, "Emergency alert 1", "emergency_metric", 1.0, 0.0
        )

        # Emergency alerts have shorter cooldown, but still limited
        await risk_manager._send_alert(
            RiskLevel.EMERGENCY, "Emergency alert 2", "emergency_metric", 1.0, 0.0
        )
...
class TestStatusAndReporting:
    ...
    async def test_performance_tracking(
        ...
        await risk_manager.check_pre_trade_risk(
            sample_order, sample_positions, 100000.0
        )
...
class TestFactoryFunction:
    ...
    def test_create_risk_manager(self, sample_config):
        ...
        manager = create_risk_manager(
            config_path=sample_config, initial_capital=50000.0
        )
...
class TestEdgeCases:
    ...
    async def test_zero_capital_handling(self, risk_manager, sample_order):
        ...
        result = await risk_manager.check_pre_trade_risk(sample_order, {}, 0.0)
