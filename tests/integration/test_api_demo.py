...
class APITester:
    ...
    def test_regime_prediction(self):
        ...
        scenarios = [
            ("Strong Uptrend", {
                ...
                "macd_signal": 0.8, "macd_histogram": 0.6, "adx": 70, "rsi": 75,
                "momentum": 0.9, "macro_surprise": 0.2, "macro_sentiment": 0.4,
                "trend_strength": 0.9, "mean_reversion": 0.1
            }),
            ("Range-bound Market", {
                "atr": 0.2, "bb_width": 0.15, "realized_vol": 0.1, "vol_ratio": 0.8,
                "macd_signal": 0.1, "macd_histogram": 0.05, "adx": 15, "rsi": 50,
                "momentum": 0.1, "macro_surprise": 0.0, "macro_sentiment": 0.0,
                "trend_strength": 0.2, "mean_reversion": 0.9
            }),
            ("High Volatility Chop", {
                "atr": 0.9, "bb_width": 0.8, "realized_vol": 0.8, "vol_ratio": 1.6,
                "macd_signal": 0.1, "macd_histogram": 0.0, "adx": 10, "rsi": 45,
                "momentum": 0.0, "macro_surprise": 0.3, "macro_sentiment": -0.2,
                "trend_strength": 0.1, "mean_reversion": 0.3
            })
        ]
        ...
        for scenario_name, features in scenarios:
            ...
            response = self.session.post(f"{self.base_url}/regime/predict", json=features)
            end_time = time.time()
            ...
            if response.status_code == 200:
                ...
                print(f"   ✅ {scenario_name}: {data['regime']} (confidence: {data['confidence']:.2f}, latency: {latency_ms:.1f}ms)")
                print(f"      Probabilities: {[(k, f'{v:.2f}') for k, v in data['probabilities'].items()]}")
    ...
    def test_current_regime(self):
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Current regime: {data['regime']} (confidence: {data['confidence']:.2f})")
            return True
    ...
    def test_regime_history(self):
        ...
        if response.status_code == 200:
            ...
            if data['history']:
                print(f"   Recent regimes: {[item['regime'] for item in data['history'][:5]]}")
        ...
        if response.status_code == 200:
            ...
            if data['transition_matrix']:
                ...
                for from_regime, transitions in data['transition_matrix'].items():
                    print(f"     {from_regime}: {[(to, f'{prob:.2f}') for to, prob in transitions.items()]}")
            else:
    ...
    def test_statistics(self):
        ...
        if response.status_code == 200:
            ...
            print(f"   Regime distribution: {[(k, f'{v:.2f}') for k, v in data['regime_distribution'].items()]}")
            return True
        else:
    ...
    def test_performance(self):
        ...
        features = {
            ...
            "macd_signal": 0.2, "macd_histogram": 0.1, "adx": 60, "rsi": 65,
            "momentum": 0.3, "macro_surprise": 0.1, "macro_sentiment": 0.2,
    ...
    def test_error_handling(self):
        ...
        response = self.session.post(f"{self.base_url}/regime/predict", json=invalid_features)
        ...
        if response.status_code == 422:
            ...
            print(f"   ❌ Expected parameter validation error, got {response.status_code}")
