...
from core.execution_algorithms import (
    ...
    ExecutionUrgency, ExecutionAlgorithm, create_execution_algorithm_manager
)
...
def test_task14_scenarios():
    ...
    try:
        ...
        num_slices = int(order.quantity / slice_size) + (1 if order.quantity % slice_size > 0 else 0)
    ...
    try:
        ...
        assert status == OrderStatus.REJECTED, f"Expected REJECTED status, got {status.value}"
    ...
    try:
        ...
        slippages = []
        for fill in fills:
            slippage_bps = abs(fill['price'] - reference_price) / reference_price * 10000
    ...
    try:
        ...
        assert final_status == OrderStatus.FAILED, f"Expected FAILED status, got {final_status.value}"
