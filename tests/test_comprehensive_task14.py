...
def test_imports():
    ...
    try:
        ...
        from core.execution_engine import (
            CoreExecutionEngine, ExecutionEngineConfig, 
        ...
        from core.order_router import (
            OrderRouter, BrokerType, MockBrokerInterface,
            create_order_router
        )
...
async def test_twap_algorithm():
    ...
    try:
        ...
        if slices and len(slices) > 0:
            ...
            print(f"   ✅ First slice: {slices[0].quantity:,.0f} units @ {slices[0].price}")
            
            # Validate slice is reasonable
            if slices[0].quantity <= parameters.max_slice_size:
                print("   ✅ Slice size within limits")
            else:
                print(f"   ⚠️  Slice size {slices[0].quantity} exceeds max {parameters.max_slice_size}")
...
async def test_pov_algorithm():
    ...
    try:
        ...
        print(f"   📝 POV order: {order.quantity:,.0f} {order.symbol} with 10% participation")
    ...
    try:
        ...
        if slices and len(slices) > 0:
            ...
            if len(slices) == 1 and slices[0].quantity == order.quantity:
                ...
                print("   ⚠️  Direct execution should generate single slice matching order quantity")
                return False
        else:
