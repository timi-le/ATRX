...
class IntegrationTestRunner:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        log_file = (
            self.output_dir
            / f"test_runner_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    async def run_scenario(self, scenario: TestScenario) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Performance score: {result['performance_score']:.2f}/100"
            )
    ...
    def _evaluate_scenario_success(
        self, scenario: TestScenario, metrics: PerformanceMetrics
    ) -> bool:
        """Evaluate if a scenario passed its success criteria."""
        criteria = [
            metrics.avg_latency_ms
            <= scenario.expected_latency_ms * 1.5,  # 50% tolerance
            metrics.throughput_tps
            >= scenario.expected_throughput_tps * 0.7,  # 30% tolerance
            (metrics.total_errors / max(metrics.total_ticks_processed, 1))
            <= scenario.expected_error_rate * 2,
    ...
    def _calculate_suite_summary(
        ...
        # Aggregate metrics
        total_ticks = sum(
            r["metrics"]["total_ticks_processed"] for r in results.values()
        )
        total_trades = sum(
            r["metrics"]["total_trades_executed"] for r in results.values()
        )
        ...
        latencies = [
            ...
            if r["metrics"]["avg_latency_ms"] > 0
        ]
        throughputs = [
            r["metrics"]["throughput_tps"]
            for r in results.values()
            if r["metrics"]["throughput_tps"] > 0
        ]
        ...
        summary = {
            ...
            "aggregate_metrics": {
                ...
                "overall_error_rate": total_errors / total_ticks
                if total_ticks > 0
                else 0,
                "avg_latency_ms": np.mean(latencies) if latencies else 0,
                "max_latency_ms": max(
                    r["metrics"]["max_latency_ms"] for r in results.values()
                )
                if results
                else 0,
            ...
            "scenario_breakdown": {
                name: {
                    ...
                    "error_rate": result["metrics"]["total_errors"]
                    / max(result["metrics"]["total_ticks_processed"], 1),
                }
    ...
    async def _generate_comprehensive_report(
        ...
        summary_file = (
            self.output_dir / f"integration_test_suite_summary_{timestamp}.json"
        )
        ...
        performance_df.to_csv(
            self.output_dir / f"performance_matrix_{timestamp}.csv", index=False
        )
                ...
                latency_data.append(
                    {
                        ...
                        "avg_latency_ms": result["metrics"]["avg_latency_ms"],
                        "max_latency_ms": result["metrics"]["max_latency_ms"],
                        "expected_latency_ms": result["scenario"][
                            "expected_latency_ms"
                        ],
                        "latency_target_ms": result["scenario"]["config"][
                            "latency_target_ms"
                        ],
                        "meets_target": result["metrics"]["avg_latency_ms"]
                        <= result["scenario"]["config"]["latency_target_ms"],
                    }
                )
        ...
        if latency_data:
            ...
            latency_df.to_csv(
                self.output_dir / f"latency_analysis_{timestamp}.csv", index=False
            )
                ...
                throughput_data.append(
                    {
                        ...
                        "expected_throughput_tps": result["scenario"][
                            "expected_throughput_tps"
                        ],
                        "target_throughput_tps": result["scenario"]["config"][
                            "throughput_target_tps"
                        ],
                        "throughput_efficiency": result["metrics"]["throughput_tps"]
                        / result["scenario"]["expected_throughput_tps"],
                        "meets_target": result["metrics"]["throughput_tps"]
                        >= result["scenario"]["config"]["throughput_target_tps"] * 0.8,
                    }
                )
        ...
        if throughput_data:
            ...
            throughput_df.to_csv(
                self.output_dir / f"throughput_analysis_{timestamp}.csv", index=False
            )
    ...
    def _generate_executive_summary(
        self, summary: dict[str, Any], results: dict[str, Any]
    ) -> str:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="FX AI-Quant Integration Test Suite Runner"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick version with reduced durations"
    )
    ...
    async def run_tests():
        ...
        if args.quick:
            ...
            for scenario in scenarios:
                scenario.config.test_duration_minutes = max(
                    1, scenario.config.test_duration_minutes // 3
                )
        ...
        print(
            f"Scenarios Passed: {results['summary']['passed_scenarios']}/{results['summary']['total_scenarios']}"
        )
        print("=" * 80)
