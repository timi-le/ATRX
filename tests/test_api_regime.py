...
from core.regime_detector import RegimeFeatures, RegimeOutput, RegimeType, RuleBasedRegimeDetector
    ...
    def test_regime_features_request_validation(self):
        ...
        with pytest.raises(ValueError):
            RegimeFeaturesRequest(
                atr=-0.1, bb_width=0.4, realized_vol=0.3, vol_ratio=1.1,
                macd_signal=0.2, macd_histogram=0.1, adx=60, rsi=65, momentum=0.3,
                macro_surprise=0.1, macro_sentiment=0.2,
                trend_strength=0.6, mean_reversion=0.3
            )
        ...
        with pytest.raises(ValueError):
            RegimeFeaturesRequest(
                atr=0.5, bb_width=0.4, realized_vol=0.3, vol_ratio=1.1,
                macd_signal=0.2, macd_histogram=0.1, adx=60, rsi=150, momentum=0.3,
                macro_surprise=0.1, macro_sentiment=0.2,
                trend_strength=0.6, mean_reversion=0.3
            )
...
class TestRegimeDetectionService:
    ...
    def service(self, mock_detector):
        ...
        return RegimeDetectionService(
            ...
            history_store=RegimeHistoryStore(),
            logger=logger
        )
...
class TestAPIPerformance:
    ...
    def test_prediction_latency(self, client, sample_features):
        ...
        assert latency_ms < 100, f"Prediction latency {latency_ms:.2f}ms exceeds 100ms requirement"
    ...
    def test_history_endpoint_performance(self, client):
        ...
        latency_ms = (end_time - start_time) * 1000
        assert latency_ms < 200, f"History endpoint latency {latency_ms:.2f}ms is too high"
