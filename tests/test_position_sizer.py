...
class TestKellyPositionSizer:
    ...
    async def test_kelly_criterion_calculation(self, position_sizer):
        ...
        kelly_fraction = position_sizer._calculate_kelly_fraction(win_probability, reward_risk_ratio)
    ...
    @pytest.mark.asyncio
    async def test_position_size_calculation(self, position_sizer, sample_signal, sample_portfolio):
        """Test complete position size calculation."""
        result = await position_sizer.calculate_position_size(sample_signal, sample_portfolio)
    ...
    @pytest.mark.asyncio
    async def test_constraints_application(self, position_sizer, sample_signal, sample_portfolio):
        """Test position sizing constraints."""
        # Test maximum position cap
        base_size = 0.10  # Above max cap
        final_size, constraints = position_sizer._apply_constraints(base_size, sample_signal, sample_portfolio)
        ...
        sample_portfolio.open_positions = [{'size': 0.05}, {'size': 0.05}, {'size': 0.05}, {'size': 0.04}]  # 0.19 total
    ...
    @pytest.mark.asyncio
    async def test_emergency_mode(self, position_sizer, sample_signal, sample_portfolio):
        ...
        assert 'emergency_mode' in result.constraints_applied or 'daily_loss_limit' in result.constraints_applied
    ...
    async def test_risk_profile_effects(self, mock_config):
        ...
        # Test conservative profile
        conservative_sizer = KellyPositionSizer(config=mock_config, risk_profile=RiskProfile.CONSERVATIVE)
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.BUY, signal_confidence=0.7,
            take_profit_pips=30.0, stop_loss_pips=20.0,
            reward_risk_ratio=1.5, win_probability=0.6, current_price=1.1000,
            volatility_atr=0.0002, timestamp=datetime.now(), strategy_name='test'
        )
        
        portfolio = PortfolioState(
            total_capital=100000.0, current_drawdown=0.0, daily_pnl=0.0,
            open_positions=[], volatility_history=[], performance_history=[]
        )
    ...
    @pytest.mark.asyncio
    async def test_performance_tracking(self, position_sizer, sample_signal, sample_portfolio):
        """Test performance metrics tracking."""
        # Make several sizing decisions
        for i in range(5):
            await position_sizer.calculate_position_size(sample_signal, sample_portfolio)
...
class TestStressTesting:
    ...
    async def test_longshot_trades(self, position_sizer):
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.BUY, signal_confidence=0.3,
            take_profit_pips=50.0, stop_loss_pips=10.0,
            reward_risk_ratio=5.0, win_probability=0.2, current_price=1.1000,
            volatility_atr=0.0002, timestamp=datetime.now(), strategy_name='longshot'
        )
    ...
    async def test_scalping_trades(self, position_sizer):
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.BUY, signal_confidence=0.9,
            take_profit_pips=10.0, stop_loss_pips=20.0,
            reward_risk_ratio=0.5, win_probability=0.8, current_price=1.1000,
            volatility_atr=0.0002, timestamp=datetime.now(), strategy_name='scalping'
        )
    ...
    async def test_high_volatility_periods(self, position_sizer):
        ...
        signal = TradeSignalInput(
            ...
            take_profit_pips=30.0, stop_loss_pips=20.0,
            reward_risk_ratio=1.5, win_probability=0.6, current_price=1.1000,
            volatility_atr=0.0008, timestamp=datetime.now(), strategy_name='high_vol'
        )
    ...
    async def test_extreme_drawdown(self, position_sizer):
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.BUY, signal_confidence=0.8,
            take_profit_pips=40.0, stop_loss_pips=20.0,
            reward_risk_ratio=2.0, win_probability=0.7, current_price=1.1000,
            volatility_atr=0.0002, timestamp=datetime.now(), strategy_name='drawdown_test'
        )
        
        portfolio = PortfolioState(
            total_capital=100000.0, current_drawdown=0.25, daily_pnl=-3000,  # 3% daily loss, below emergency threshold
            open_positions=[], volatility_history=[], performance_history=[]
        )
        ...
        assert result.position_size <= 0.05  # Should respect max cap
        assert result.confidence < 0.7  # Confidence should be reduced due to severe drawdown
...
class TestIntegrationScenarios:
    ...
    async def test_trending_market_scenario(self, real_position_sizer):
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.BUY, signal_confidence=0.85,
            take_profit_pips=50.0, stop_loss_pips=25.0, reward_risk_ratio=2.0,
            win_probability=0.65, current_price=1.1000, volatility_atr=0.0003,
            timestamp=datetime.now(), strategy_name='breakout_trend'
        )
        
        portfolio = PortfolioState(
            total_capital=100000.0, current_drawdown=0.0, daily_pnl=500.0,
    ...
    async def test_mean_reverting_scenario(self, real_position_sizer):
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.SELL, signal_confidence=0.7,
            take_profit_pips=20.0, stop_loss_pips=30.0, reward_risk_ratio=0.67,
            win_probability=0.75, current_price=1.1000, volatility_atr=0.0001,
            timestamp=datetime.now(), strategy_name='grid_martingale'
        )
        
        # Create volatility history that will make 0.0001 appear as low volatility
        volatility_history = [0.0003, 0.0004, 0.0005, 0.0003, 0.0004] * 10  # 50 values, higher than current
        
        portfolio = PortfolioState(
            total_capital=100000.0, current_drawdown=0.03, daily_pnl=-200.0,
            open_positions=[],
        ...
        assert result.volatility_regime in ['low', 'normal']  # Accept either since it depends on history
        assert result.drawdown_level == 'none'  # 3% drawdown is below 5% mild threshold
        assert any('max_position_cap' in constraint for constraint in result.constraints_applied)  # Should hit position cap
    ...
    async def test_portfolio_constraint_scenario(self, real_position_sizer):
        ...
        signal = TradeSignalInput(
            symbol='EURUSD', side=OrderSide.BUY, signal_confidence=0.8,
            take_profit_pips=36.0, stop_loss_pips=20.0,
            reward_risk_ratio=1.8, win_probability=0.6, current_price=1.1000,
            volatility_atr=0.0002, timestamp=datetime.now(), strategy_name='test'
        )
        ...
        assert result.position_size <= 0.031  # Max allowed to stay under 0.20 with some tolerance
