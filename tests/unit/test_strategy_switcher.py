...
from core.strategy_switcher import StrategySwitcher, StrategyDecision
...
def mock_strategies():
    ...
    trend_signal = Signal(
        symbol='EUR/USD', side=OrderSide.BUY, size=1.0, order_type='market',
        strength=0.8, confidence=0.7, strategy_name='breakout_trend', timestamp=datetime.now()
    )
    trend_strategy = MagicMock()
    trend_strategy.generate_signal = AsyncMock(return_value=trend_signal)
    
    mr_signal = Signal(
        symbol='EUR/USD', side=OrderSide.SELL, size=0.5, order_type='market',
        strength=0.6, confidence=0.65, strategy_name='grid_martingale', timestamp=datetime.now()
    )
...
async def test_mean_reverting_regime_selects_correct_strategy(strategy_switcher):
    ...
    regime = RegimeOutput(regime=RegimeType.MEAN_REVERTING, confidence=0.9, probabilities={})
...
async def test_low_confidence_regime_produces_no_signal(strategy_switcher):
    ...
    regime = RegimeOutput(regime=RegimeType.TRENDING, confidence=0.5, probabilities={}) # Below threshold 0.6
...
async def test_ml_weighting_scales_signal_size(strategy_switcher):
    ...
    decision = await strategy_switcher.choose_strategy(regime=regime, ml_prediction=ml_pred)
