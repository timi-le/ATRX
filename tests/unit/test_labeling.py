...
def sample_data():
    ...
    close = np.array([100, 102, 105, 103, 106, 104, 108, 110, 107, 109, 112, 110, 108, 105, 103, 100])
...
def test_upper_barrier_hit(sample_data):
    ...
    labels = generate_triple_barrier_labels(sample_data, upper_mult=2.0, lower_mult=2.0, vertical_barrier_bars=5)
    assert labels[0] == 1
...
def test_lower_barrier_hit():
    ...
    labels = generate_triple_barrier_labels(data, upper_mult=3.0, lower_mult=1.5, vertical_barrier_bars=4)
    assert labels[0] == -1
...
def test_vertical_barrier_hit():
    ...
    labels = generate_triple_barrier_labels(data, upper_mult=2.0, lower_mult=2.0, vertical_barrier_bars=4)
    assert labels[0] == 0
...
def test_last_values_are_nan(sample_data):
    ...
    labels = generate_triple_barrier_labels(sample_data, vertical_barrier_bars=vertical_barrier_bars)
