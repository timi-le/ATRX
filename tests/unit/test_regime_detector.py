...
class TestRegimeFeatures:
    ...
    def test_from_array_conversion(self):
        ...
        array = np.array(
            [0.5, 0.4, 0.3, 1.1, 0.2, 0.1, 60, 65, 0.3, 0.1, 0.2, 0.6, 0.3]
        )
...
class TestRuleBasedRegimeDetector:
    ...
    def test_fit_method(self):
        ...
        self.logger.info.assert_called_with(
            "Rule-based detector fit called with 100 samples"
        )
    ...
    def test_trending_regime_detection(self):
        ...
        assert (
            result.probabilities[RegimeType.TRENDING]
            > result.probabilities[RegimeType.MEAN_REVERTING]
        )
        assert (
            result.probabilities[RegimeType.TRENDING]
            > result.probabilities[RegimeType.CHOPPY]
        )
...
class TestStatisticalRegimeDetector:
    ...
    def test_fit_and_map_clusters(self):
        ...
        trending_features = create_sample_features(
            n_samples=50, regime_type=RegimeType.TRENDING
        )
        mr_features = create_sample_features(
            n_samples=50, regime_type=RegimeType.MEAN_REVERTING
        )
        choppy_features = create_sample_features(
            n_samples=50, regime_type=RegimeType.CHOPPY
        )
        ...
        self.logger.info.assert_any_call(
            "GMM model fitted.", inertia=self.detector.model.lower_bound_
        )
    ...
    def test_predict_and_proba_consistency(self):
        ...
        predict_features = create_sample_features(
            n_samples=1, regime_type=RegimeType.TRENDING
        )[0]
    ...
    def test_empty_features_fit(self):
        ...
        self.detector.fit([])
        self.logger.warning.assert_called_with(
            "fit called with no features. Model cannot be trained."
        )
...
class TestRegimeOutput:
    ...
    def test_regime_output_optional_fields(self):
        ...
        output = RegimeOutput(
            regime=RegimeType.TRENDING, confidence=0.8, probabilities=probabilities
        )
