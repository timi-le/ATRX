...
from trainers.retraining_scheduler import (
    RetrainingScheduler, DataDriftDetector, ModelVersionManager,
    ModelVersion, RetrainingTrigger, DriftDetectionResult
)
...
class TestDataDriftDetector:
    ...
    def test_drift_detection_no_drift(self, drift_detector):
        ...
        current_data = drift_detector.reference_data + np.random.randn(*drift_detector.reference_data.shape) * 0.001
...
class TestRetrainingScheduler:
    ...
    def test_should_retrain_performance(self, scheduler):
        ...
        # Mock active version
        scheduler.version_manager.get_active_version = Mock(return_value=Mock(validation_score=0.80))
