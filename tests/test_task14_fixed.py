...
def test_imports():
    ...
    try:
        from core.execution_engine import (
            CoreExecutionEngine, create_execution_engine
        )
...
async def test_task14_scenario_2_pov():
    ...
    try:
        from core.execution_algorithms import (
            create_execution_algorithm_manager, MarketData, 
            ExecutionParameters, ExecutionAlgorithm, ExecutionUrgency
        )
        from core.interfaces.trading_interfaces import Order, OrderSide, OrderType
        ...
        # Validate adaptation
        assert adaptation_factor >= 3.0, f"Expected significant adaptation, got {adaptation_factor:.1f}x"
...
async def test_task14_scenario_3_direct():
    ...
    try:
        from core.execution_engine import create_execution_engine
        from core.interfaces.trading_interfaces import Order, OrderSide, OrderType, OrderStatus
        ...
        try:
            ...
            status = await asyncio.wait_for(
                engine.get_order_status(order_id),
                timeout=2.0
            )
            print(f"   ✅ Status: {status.value}")
            
            # Validate direct execution characteristics
            assert execution_time_ms < 5000, f"Direct execution too slow: {execution_time_ms:.2f}ms"
            assert status in [OrderStatus.PENDING, OrderStatus.FILLED, OrderStatus.REJECTED], f"Unexpected status: {status.value}"
...
async def test_task14_scenario_4_rejection():
    ...
    try:
        ...
        try:
            ...
            # Validate rejection
            assert status == OrderStatus.REJECTED, f"Expected REJECTED, got {status.value}"
    ...
    try:
        ...
        try:
            metrics = await asyncio.wait_for(
                engine.calculate_execution_quality(order, fills),
                timeout=3.0
            )
