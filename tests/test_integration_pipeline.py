...
class MarketDataSimulator:
    ...
    def generate_realistic_tick(
        self, symbol: str, base_price: float, volatility: float = 0.001
    ) -> MarketTick:
        ...
        while self.is_running:
            ...
            for symbol in self.config.instruments:
                tick = self.generate_realistic_tick(
                    symbol, current_prices[symbol], volatility=0.0005  # Base volatility
                )
...
class SystemIntegrationTester:
    ...
    def _get_mock_response(self, component_name: str, method_name: str, args):
        ...
        mock_responses = {
            ...
            ("regime_detector", "detect_regime"): np.random.choice(
            ...
            ("strategy_switcher", "get_signal"): np.random.choice(
                ["buy", "sell", None]
            ),
            ...
            try:
                ...
                if self.start_time:
                    ...
                    self.metrics.throughput_tps = (
                        self.metrics.total_ticks_processed / elapsed
                    )
    ...
    def _calculate_final_metrics(self):
        ...
        if self.start_time:
            ...
            self.metrics.throughput_tps = (
                self.metrics.total_ticks_processed / test_duration
            )
    ...
    async def _generate_test_report(self):
        ...
        if self.latency_measurements:
            ...
            latency_df.to_csv(
                report_dir / f"latency_report_{timestamp}.csv", index=False
            )
    ...
    def _evaluate_test_success(self) -> bool:
        ...
        success_criteria = [
            ...
            self.metrics.throughput_tps
            >= self.config.throughput_target_tps * 0.8,  # 80% tolerance
            self.metrics.max_drawdown < self.config.max_drawdown_threshold,
            self.metrics.total_errors
            < self.metrics.total_ticks_processed * 0.0001,  # <0.01% error rate
        ...
        self.logger.info(
            f"Test success: {passed_criteria}/{total_criteria} criteria passed"
        )
...
class IntegrationTestSuite(unittest.TestCase):
    ...
    def test_basic_integration(self):
        ...
        async def run_test():
            ...
            self.assertLess(
                metrics.total_errors / max(metrics.total_ticks_processed, 1), 0.01
            )
    ...
    def test_stress_scenarios(self):
        ...
        async def run_stress_test():
            ...
            self.assertLess(
                metrics.total_errors / max(metrics.total_ticks_processed, 1), 0.05
            )
...
def run_integration_test_cli():
    ...
    parser.add_argument(
        "--tick-rate", type=int, default=1000, help="Tick rate per second"
    )
    parser.add_argument(
        "--instruments",
        nargs="+",
    ...
    async def main():
        ...
        print(
            f"Error Rate: {metrics.total_errors/max(metrics.total_ticks_processed,1):.4%}"
        )
