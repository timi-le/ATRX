...
class TestStrategySwitcher:
    ...
    def switcher(self, mock_config):
        ...
        with patch("core.strategy_switcher.GridMartingaleStrategy"), patch(
            "core.strategy_switcher.BreakoutTrendStrategy"
    ...
    async def test_ml_prediction_weighting(self, switcher):
        ...
        weighted_signal = switcher._apply_ml_weighting(
            original_signal, agreeing_prediction
        )
        ...
        weighted_signal = switcher._apply_ml_weighting(
            fresh_signal, disagreeing_prediction
        )
    ...
    async def test_ensemble_mode(self, switcher):
        ...
        for strategy_name, signal in mock_signals.items():
            switcher.strategies[strategy_name].generate_signal = AsyncMock(
                return_value=signal
            )
    ...
    async def test_strategy_parameter_updates(self, switcher):
        ...
        switcher.strategies[
            "grid_martingale"
        ].update_parameters.assert_called_once_with(new_params)
