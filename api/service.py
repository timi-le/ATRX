...
class RegimeHistoryStore:
    ...
    async def get_transition_counts(self) -> dict[str, dict[str, int]]:
        ...
        async with self._lock:
            ...
            transition_counts = {
                regime: {target: 0 for target in regimes} for regime in regimes
            }
...
class RegimeDetectionService:
    ...
    def __init__(
        ...
        history_store: RegimeHistoryStore | None = None,
        ...
        self._transition_matrix_cache: dict[str, Any] | None = None
        self._transition_matrix_last_update: datetime | None = None
        self._cache_ttl_seconds = 300  # 5 minutes

        self.logger.info("RegimeDetectionService initialized")

    async def predict_regime(
        self, features_request: RegimeFeaturesRequest
    ) -> RegimeResponse:
    ...
    async def get_current_regime(self) -> RegimeResponse | None:
        ...
        try:
            ...
            return RegimeResponse(
                ...
                probabilities={
                    RegimeTypeAPI(k.value): v for k, v in latest.probabilities.items()
                },
    ...
    async def get_regime_history(
        ...
        try:
            if start_time and end_time:
                history = await self.history_store.get_history_by_time(
                    start_time, end_time
                )
            else:
    ...
    async def get_transition_matrix(self) -> TransitionMatrixResponse:
        ...
        try:
            ...
            if (
                ...
                and self._transition_matrix_last_update
                and (now - self._transition_matrix_last_update).total_seconds()
    ...
    async def simulate_regime_data(self, num_samples: int = 50) -> None:
        ...
        for regime_type in RegimeType:
            samples = create_sample_features(
                n_samples=num_samples // 3, regime_type=regime_type
            )
        ...
        self.logger.info(
            f"Simulation complete. History size: {self.history_store.size()}"
        )


# Global service instance
_service_instance: RegimeDetectionService | None = None
