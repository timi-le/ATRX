...
class RegimeFeaturesRequest(BaseModel):
    ...
    bb_width: float = Field(
        ..., ge=0, le=1, description="Bollinger Band width (normalized)"
    )
    realized_vol: float = Field(
        ..., ge=0, le=1, description="Realized volatility (normalized)"
    )
    vol_ratio: float = Field(
        ..., ge=0, le=5, description="Current vol / historical vol"
    )
    ...
    # Macro features (2)
    macro_surprise: float = Field(
        ..., ge=-1, le=1, description="Economic surprise index"
    )
    ...
    mean_reversion: float = Field(
        ..., ge=0, le=1, description="Mean reversion tendency"
    )
...
class RegimeResponse(BaseModel):
    ...
    probabilities: dict[RegimeTypeAPI, float] = Field(
        ..., description="Probability for each regime"
    )
    transition_prob: float | None = Field(
        None, ge=0, le=1, description="Probability of regime transition"
    )
    features_used: list[str] | None = Field(
        None, description="Features that contributed most"
    )
...
class TransitionMatrixResponse(BaseModel):
    ...
    transition_matrix: None | (
        dict[RegimeTypeAPI, dict[RegimeTypeAPI, float]]
    ) = Field(None, description="Transition probabilities between regimes")
    model_type: str = Field(
        ..., description="Type of model used (e.g., 'hmm', 'rule-based')"
    )
    last_updated: datetime = Field(..., description="When the matrix was last computed")
    sample_size: int | None = Field(
        None, description="Number of samples used to compute matrix"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                "transition_matrix": {
                    "trending": {"trending": 0.7, "mean_reverting": 0.2, "choppy": 0.1},
                    "mean_reverting": {
                        "trending": 0.3,
                        "mean_reverting": 0.6,
                        "choppy": 0.1,
                    },
                    "choppy": {"trending": 0.4, "mean_reverting": 0.3, "choppy": 0.3},
                },
                "model_type": "hmm",
...
class HealthResponse(BaseModel):
    ...
    detector_ready: bool = Field(
        ..., description="Whether the regime detector is ready"
    )
