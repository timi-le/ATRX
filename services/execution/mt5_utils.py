...
class MT5Utils:
    ...
    def calculate_pip_value(
        ...
        try:
            ...
            if symbol_info.currency_profit != account_currency:
                conversion_rate = self.get_conversion_rate(symbol_info.currency_profit, account_currency)
                pip_value *= conversion_rate
            
            return pip_value
            
        except Exception as e:
            self.logger.error("Error calculating pip value", symbol=symbol, error=str(e))
            return 0.0
    ...
    def get_conversion_rate(self, from_currency: str, to_currency: str) -> float:
        ...
        try:
            ...
            self.logger.warning("Cannot find conversion rate", from_currency=from_currency, to_currency=to_currency)
            return 1.0
    ...
    def _is_time_in_session(self, current_time: str, open_time: str, close_time: str) -> bool:
    ...
    def _get_next_session_time(self, time_type: str, from_time: datetime) -> Optional[datetime]:
    ...
    def get_spread_info(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            spread_percentage = (spread_pips / symbol_info.bid) * 100 if symbol_info.bid > 0 else 0
    ...
    def get_trading_statistics(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'total_session_orders': symbol_info.session_buy_orders + symbol_info.session_sell_orders,
                'buy_sell_ratio': (
                    symbol_info.session_buy_orders / symbol_info.session_sell_orders 
                    if symbol_info.session_sell_orders > 0 else 0
                ),
            ...
            self.logger.error("Error getting trading statistics", symbol=symbol, error=str(e))
            return {}


def create_mt5_utils(logger: Optional[structlog.stdlib.BoundLogger] = None) -> MT5Utils:
