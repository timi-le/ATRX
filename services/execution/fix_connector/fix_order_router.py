...
from .fix_client import FIXClient, load_fix_config
from .fix_message_utils import FIXMessage
...
class FIXOrderRouter(BrokerInterface):
    ...
    def __init__(
        ...
        self.client_order_id_mapping: dict[
            str, str
        ] = {}  # FIX client order ID -> internal order ID
        ...
        self.order_update_callback: None | (
            Callable[[str, OrderStatus, dict[str, Any]], None]
        ) = None
        self.execution_callback: None | (
            Callable[[str, float, float, datetime], None]
        ) = None
    ...
    def _setup_message_handlers(self):
        ...
        self.fix_client.register_message_handler(
            "j", self._handle_business_message_reject
        )
    ...
    async def submit_order(self, order: Order) -> dict[str, Any]:
        ...
        try:
            ...
            fix_time_in_force = self._convert_time_in_force_to_fix(
            ...
            return {"success": False, "order_id": order.order_id, "error": str(e)}
    ...
    def get_order_status(self, order_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "last_update": order_mapping.last_update.isoformat()
            if order_mapping.last_update
            else None,
    ...
    def get_all_orders(self) -> list[dict[str, Any]]:
        ...
        return [
            self.get_order_status(order_id) for order_id in self.active_orders.keys()
        ]
    ...
    def get_positions(self) -> list[Position]:
        ...
        for order_mapping in self.active_orders.values():
            ...
            if order_mapping.filled_quantity > 0:
                side_multiplier = (
                    1 if order_mapping.side == "1" else -1
                )  # 1=Buy, 2=Sell
    ...
    def _handle_execution_report(self, message: FIXMessage):
        ...
        try:
            ...
            if not internal_order_id:
                self.logger.warning(
                    "Received execution report for unknown order",
                    cl_ord_id=exec_report.cl_ord_id,
                )
                return
            ...
            if not order_mapping:
                self.logger.warning(
                    "Order mapping not found", order_id=internal_order_id
                )
                return
            ...
            order_mapping.execution_reports.append(
                {
                    ...
                    "transact_time": exec_report.transact_time.isoformat()
                    if exec_report.transact_time
                    else None,
            ...
            internal_status = self._convert_fix_status_to_internal(
                exec_report.ord_status
            )
    ...
    def _convert_side_to_fix(self, side: OrderSide) -> str:
        ...
        mapping = {OrderSide.BUY: "1", OrderSide.SELL: "2"}
    ...
    def _convert_time_in_force_to_fix(self, tif: str) -> str:
        ...
        mapping = {"DAY": "0", "GTC": "1", "IOC": "3", "FOK": "4"}
    ...
    def set_order_update_callback(
        self, callback: Callable[[str, OrderStatus, dict[str, Any]], None]
    ):
        """Set order update callback."""
        self.order_update_callback = callback

    def set_execution_callback(
        self, callback: Callable[[str, float, float, datetime], None]
    ):
