...
from .fix_message_utils import FIXMessage, FIXMessageUtils, FIXVersion
...
class FIXSessionStats:
    ...
    last_heartbeat_sent: datetime | None = None
    last_heartbeat_received: datetime | None = None
...
class FIXClient:
    ...
    def __init__(
        ...
        fix_version = (
            FIXVersion.FIX_4_2
            if config.fix_version == "FIX.4.2"
            else FIXVersion.FIX_4_4
        )
    ...
    async def connect(self) -> bool:
        ...
        try:
            self.logger.info(
                "Connecting to FIX server", host=self.config.host, port=self.config.port
            )
            ...
            self.socket.setsockopt(
                socket.SOL_SOCKET, socket.SO_SNDBUF, self.config.send_buffer_size
            )
            self.socket.setsockopt(
                socket.SOL_SOCKET, socket.SO_RCVBUF, self.config.recv_buffer_size
            )
            ...
            while (
                self.state == FIXSessionState.LOGON_SENT and time.time() < logon_timeout
            ):
    ...
    async def send_message(self, message: str) -> bool:
        ...
        try:
            if self.state not in [
                FIXSessionState.LOGGED_ON,
                FIXSessionState.LOGON_SENT,
            ]:
                self.logger.warning(
                    "Cannot send message - not logged on", state=self.state.value
                )
                return False
            ...
            if self.config.log_outgoing:
                self.logger.debug(
                    "Sent FIX message", message=message.replace("\x01", "|")
                )
    ...
    async def _process_received_message(self, message_str: str):
        ...
        try:
            ...
            if self.config.log_incoming:
                self.logger.debug(
                    "Received FIX message", message=message_str.replace("\x01", "|")
                )
            ...
            if self.message_utils.is_admin_message(message.msg_type):
                ...
                if handler:
                    ...
                    self.logger.warning(
                        "No handler for admin message", msg_type=message.msg_type
                    )
            else:
                ...
                if handler:
                    ...
                    self.logger.warning(
                        "No handler for application message", msg_type=message.msg_type
                    )
        ...
        while self._running:
            try:
                ...
                if self.state == FIXSessionState.LOGGED_ON:
                    ...
                    now = time.time()
                    if (
                        not self.last_heartbeat_time
                        or now - self.last_heartbeat_time
                    ...
                    if (
                        self.stats.last_message_received
                        and now - self.stats.last_message_received.timestamp()
    ...
    async def _send_logout(self, text: str | None = None):
        """Send logout message."""
        try:
            self.state = FIXSessionState.LOGOUT_SENT
    ...
    async def _send_heartbeat(self, test_req_id: str | None = None):
    ...
    def _handle_test_request(self, message: FIXMessage):
        ...
        self.logger.debug(
            "Test request received, sending heartbeat", test_req_id=test_req_id
        )
    ...
    def _handle_resend_request(self, message: FIXMessage):
        ...
        self.logger.warning(
            "Resend request received", begin_seq_no=begin_seq_no, end_seq_no=end_seq_no
        )
    ...
    async def _schedule_reconnect(self):
        ...
        delay = min(
            ...
            * (self.config.reconnect_backoff ** (self.stats.reconnection_attempts - 1)),
    ...
    def register_message_handler(
        self, msg_type: str, handler: Callable[[FIXMessage], None]
    ):
    ...
    def get_session_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "session_start_time": self.stats.session_start_time.isoformat()
