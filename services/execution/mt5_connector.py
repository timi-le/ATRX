...
class MT5Connector:
    ...
    def _load_config(self, config_path: str) -> MT5Config:
        ...
        try:
            ...
            if config_file.exists():
                ...
                self.logger.warning(
                    "Config file not found, using defaults", path=config_path
                )
                return MT5Config(
                    login=0,  # Must be set
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "Connecting to MT5", login=self.config.login, server=self.config.server
            )
        ...
        try:
            ...
            if self.status != MT5ConnectionStatus.CONNECTED:
                return MT5ExecutionResult(
                    success=False, error_description="MT5 not connected"
                )
            ...
            if symbol_info is None:
                return MT5ExecutionResult(
                    success=False, error_description=f"Symbol {order.symbol} not found"
                )
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                if order.price and result.price:
                    slippage = (
                        abs(result.price - order.price) / order.price * 10000
                    )  # bps
                    self.total_slippage += slippage

                # Update average execution time
                self.avg_execution_time = (
                    self.avg_execution_time * (self.successful_orders - 1)
                    + execution_time
            ...
            execution_time = (time.perf_counter() - start_time) * 1000
            self.logger.error(
                "Order submission error", order_id=order.order_id, error=str(e)
            )
            return MT5ExecutionResult(
                success=False, execution_time=execution_time, error_description=str(e)
            )
    ...
    def _prepare_trade_request(self, order: Order, symbol_info) -> dict[str, Any]:
        ...
        if order.order_type == OrderType.MARKET:
            ...
            order_type = (
                MT5OrderType.BUY.value
                if order.side == OrderSide.BUY
                else MT5OrderType.SELL.value
            )
        ...
        volume = min(
            max(order.quantity / 100000, self.config.min_volume), self.config.max_volume
        )
    ...
    async def cancel_order(self, order_id: str) -> MT5ExecutionResult:
        ...
        try:
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                error_desc = (
                    self._get_error_description(result.retcode)
                    if result
                    else "Unknown error"
                )
                return MT5ExecutionResult(
    ...
    async def close_position(
        self, symbol: str, volume: float | None = None
    ) -> MT5ExecutionResult:
        """Close a position."""
        try:
            # Get position info
            ...
            if not positions:
                return MT5ExecutionResult(
                    success=False, error_description=f"No position found for {symbol}"
                )
            ...
            if symbol_info is None:
                return MT5ExecutionResult(
                    success=False, error_description=f"Symbol {symbol} not found"
                )
    ...
    def get_position(self, symbol: str) -> MT5Position | None:
