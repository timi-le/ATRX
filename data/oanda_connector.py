...
class OandaConnector(BaseFXConnector):
    ...
    def __init__(self, config: OandaConfig, logger: Optional[structlog.stdlib.BoundLogger] = None):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            stream_timeout = aiohttp.ClientTimeout(total=self.oanda_config.stream_timeout)
    ...
    async def _verify_connection(self) -> None:
        ...
        async with self.session.get(url) as response:
            ...
            self.logger.info("OANDA account verified",
    ...
    async def stream(self) -> AsyncGenerator[MarketData, None]:
        ...
        self.logger.info("Starting OANDA price stream", 
                        instruments=instruments,
                        url=url)
        ...
        try:
            async with self.stream_session.get(url, params=params) as response:
                if response.status != 200:
                    error_data = await response.text()
                    raise RuntimeError(f"OANDA stream error {response.status}: {error_data}")
                ...
                async for line in response.content:
                    ...
                    try:
                        ...
                        self.logger.warning("Failed to parse OANDA stream data", 
                                          error=str(e), 
                                          data=line_str[:100])
                        continue
                    except Exception as e:
                        self.logger.error("Error processing OANDA stream data", error=str(e))
                        continue
    ...
    def _parse_price_data(self, data: Dict) -> Optional[MarketData]:
        ...
        try:
            ...
            self.logger.warning("Failed to parse OANDA price data", 
                              error=str(e), 
                              data=data)
            return None
    ...
    async def get_historical_data(
        ...
        self.logger.info("Fetching OANDA historical data",
                        symbol=symbol,
        ...
        try:
            async with self.session.get(url, params=params) as response:
                ...
                self.logger.info("Retrieved OANDA historical data",
                               symbol=symbol,
            ...
            self.logger.error("Failed to get OANDA historical data", 
                            symbol=symbol, 
                            error=str(e))
            raise
    
    def _parse_candle_data(self, candle: Dict, symbol: str, timeframe: str) -> Optional[OHLCV]:
        ...
        try:
            ...
            self.logger.warning("Failed to parse OANDA candle data", 
                              error=str(e), 
                              data=candle)
            return None
