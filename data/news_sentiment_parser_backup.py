...
class SimpleSentimentAnalyzer(SentimentAnalyzer):
    ...
    def __init__(self, logger: Optional[structlog.stdlib.BoundLogger] = None):
        ...
        self.positive_words = {
            'bullish', 'positive', 'rise', 'rising', 'up', 'increase', 'growth', 'gains',
            'stronger', 'strengthen', 'boost', 'rally', 'surged', 'jumped', 'climbed',
            'optimistic', 'confident', 'recovery', 'improve', 'improving', 'outperform',
            'exceed', 'beat', 'better', 'good', 'strong', 'robust', 'solid', 'healthy'
        }
        
        self.negative_words = {
            'bearish', 'negative', 'fall', 'falling', 'down', 'decrease', 'decline', 'losses',
            'weaker', 'weaken', 'drop', 'plunge', 'tumbled', 'crashed', 'slumped',
            'pessimistic', 'concern', 'worried', 'recession', 'crisis', 'deteriorate',
            'miss', 'below', 'worse', 'bad', 'weak', 'fragile', 'poor', 'disappointing'
        }
        
        # Currency-specific keywords
        self.currency_keywords = {
            Currency.USD: [
                'dollar', 'usd', 'federal reserve', 'fed', 'fomc', 'united states', 'us economy',
                'treasury', 'yellen', 'powell', 'america', 'american'
            ],
            Currency.EUR: [
                'euro', 'eur', 'ecb', 'european central bank', 'eurozone', 'eu', 'lagarde',
                'european union', 'germany', 'france', 'italy', 'spain'
            ],
            Currency.GBP: [
                'pound', 'sterling', 'gbp', 'bank of england', 'boe', 'uk', 'britain',
                'british', 'england', 'bailey', 'brexit'
            ],
            Currency.JPY: [
                'yen', 'jpy', 'bank of japan', 'boj', 'japan', 'japanese', 'tokyo',
                'kuroda', 'ueda'
            ],
            Currency.AUD: [
                'australian dollar', 'aud', 'rba', 'reserve bank australia', 'australia',
                'aussie', 'sydney', 'canberra'
            ],
            Currency.CAD: [
                'canadian dollar', 'cad', 'bank of canada', 'boc', 'canada', 'loonie',
                'ottawa', 'toronto'
            ],
            Currency.CHF: [
                'swiss franc', 'chf', 'snb', 'swiss national bank', 'switzerland',
                'swiss', 'zurich', 'geneva'
            ],
            Currency.NZD: [
                'new zealand dollar', 'nzd', 'rbnz', 'reserve bank new zealand',
...
class MockNewsProvider(NewsProvider):
    ...
    async def get_live_news(self) -> List[NewsEvent]:
        ...
        recent_news = [
            event for event in self.mock_news_data
...
class RSSNewsProvider(NewsProvider):
        ...
        try:
            ...
            async with self.session.get(url, timeout=30) as response:
                if response.status != 200:
                    self.logger.warning(f"Failed to fetch RSS feed from {source}: HTTP {response.status}")
                    return []
            ...
            for entry in feed.entries:
                ...
                currencies_mentioned = self.sentiment_analyzer.extract_currencies(f"{headline} {content}")
                
                # Analyze sentiment
                sentiment_score, confidence = await self.sentiment_analyzer.analyze_sentiment(f"{headline} {content}")
                ...
                event = NewsEvent(
                    ...
                    relevance=len(currencies_mentioned) * 0.2 + 0.5,  # Simple relevance scoring
    ...
    async def get_news_feed(
        ...
        self.logger.info(f"Retrieved {len(final_events)} unique news events from RSS feeds")
        return final_events
...
class NewsAPIProvider(NewsProvider):
    ...
    async def get_news_feed(
        ...
        if not self.api_key:
            ...
            return await mock_provider.get_news_feed(start_date, end_date, currencies, keywords)
...
class NewsSentimentParser:
    ...
    async def get_news(
        ...
        for provider in self.news_providers:
            try:
                news = await provider.get_news_feed(start_date, end_date, currencies, keywords)
    ...
    async def analyze_sentiment_batch(self, texts: List[str]) -> List[Tuple[float, float]]:
    ...
    def remove_provider(self, provider_class: type):
        ...
        self.news_providers = [
            p for p in self.news_providers 
