...
class MockEconomicDataProvider(MacroDataProvider):
    ...
    def _generate_mock_historical_data(self):
        ...
        for day_offset in range(30):
            ...
            for i in range(num_events):
                ...
                forecast = template["typical_forecast"] + random.gauss(
                    0, template["volatility"] * 0.2
                )
        ...
        self.logger.info(
            f"Generated {len(self.historical_events)} mock historical events"
        )
    ...
    async def update_event_actual(self, event_id: str, actual_value: float) -> bool:
        ...
        if event_id in self.events_cache:
            ...
            self.logger.info(
                f"Updated event {event_id} with actual value {actual_value}"
            )
            return True
...
class ForexFactoryDataProvider(MacroDataProvider):
    ...
    async def get_economic_calendar(
        ...
        self.logger.warning(
            "ForexFactoryDataProvider is not yet implemented - using mock data"
        )
...
class TradingEconomicsProvider(MacroDataProvider):
    ...
    async def get_economic_calendar(
        ...
        if not self.api_key:
            self.logger.warning(
                "No API key provided for Trading Economics - using mock data"
            )
            mock_provider = MockEconomicDataProvider(self.logger)
            return await mock_provider.get_economic_calendar(
                start_date, end_date, currencies, impact_levels
        ...
        self.logger.warning(
            "TradingEconomicsProvider API integration not yet implemented"
        )
        return []
...
class EconomicCalendarLoader:
    ...
    def remove_provider(self, provider_class: type):
        ...
        self.providers = [
            p for p in self.providers if not isinstance(p, provider_class)
        ]
