...
class MockMarketDataGenerator:
    ...
    def generate_price_movement(self) -> float:
        ...
        reversion_component = (
            -(self.current_price - self.initial_price) * reversion_strength
        )
...
class MockFXConnector(BaseFXConnector):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "Connected to mock FX provider", symbols=list(self.generators.keys())
            )
            return True
    ...
    async def stream(self) -> AsyncGenerator[MarketData, None]:
        ...
        self.logger.info(
            "Starting mock data stream", symbols=list(self.generators.keys())
        )
        self._stop_streaming = False
    ...
    async def get_historical_data(
        ...
        for i in range(num_bars):
            ...
            bar_time = current_time - timedelta(
                minutes=timeframe_minutes * (num_bars - i - 1)
            )
    ...
    def simulate_market_event(
        self, symbol: str, event_type: str, magnitude: float = 0.001
    ) -> None:
        """
