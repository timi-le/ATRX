...
class FXTradingMetrics:
    ...
    def _initialize_metrics(self):
        ...
        self.daily_pnl = Gauge(
            "fxai_daily_pnl", "Daily PnL in USD", registry=self.registry
        )
        ...
        self.memory_usage = Gauge(
            "fxai_memory_usage_mb", "Memory usage in MB", registry=self.registry
        )

        self.cpu_usage = Gauge(
            "fxai_cpu_usage_percent", "CPU usage percentage", registry=self.registry
        )
        ...
        self.sharpe_ratio = Gauge(
            "fxai_sharpe_ratio", "Current Sharpe ratio", registry=self.registry
        )
        ...
        self.system_info = Info(
            "fxai_system_info", "System information", registry=self.registry
        )
    ...
    def start_server(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"🚀 Metrics server running on http://{self.config.host}:{self.config.port}/metrics"
            )
    ...
    def update_pnl(
        self, equity: float, daily_pnl: float = None, unrealized_pnl: float = None
    ) -> None:
    ...
    def record_trade(
        self, symbol: str, side: str, status: str, pnl: float = None
    ) -> None:
    ...
    def update_position_exposure(
        self, symbol: str, side: str, exposure_usd: float
    ) -> None:
    ...
    def record_strategy_signal(
        self, strategy: str, signal_type: str, symbol: str
    ) -> None:
    ...
    def record_exception(self, exception_type: str, component: str) -> None:
        ...
        self.exception_counter.labels(
            exception_type=exception_type, component=component
        ).inc()
    ...
    def update_from_backtest_results(self, results_df: Any) -> None:
        ...
        if hasattr(results_df, "empty") and not results_df.empty:
            ...
            (results_df.get("pnl", pd.Series()) > 0).mean()
...
def update_trading_session_metrics(
    equity: float,
    daily_pnl: float,
    ...
    )  # Assuming current is also max for simplicity
