...
async def strategy_comparison_example():
    ...
    strategies_to_test = [
        ...
        (
            "Balanced Mix",
            ["grid_martingale", "breakout_trend"],
    ...
    if len(results) > 1:
        ...
        print(
            f"{'Strategy':<20} {'Return':<10} {'Sharpe':<8} {'Max DD':<8} {'Trades':<8}"
        )
        print("-" * 60)
        ...
        for strategy_name, result in results.items():
            ...
            print(
                f"{strategy_name:<20} {perf['total_return']:>8.2%} "
...
async def performance_analysis_example():
    ...
    if not rolling_metrics.empty:
        ...
        print(
            f"  Average Rolling Return: {rolling_metrics['rolling_return'].mean():.2%}"
        )
        print(
            f"  Average Rolling Sharpe: {rolling_metrics['rolling_sharpe'].mean():.2f}"
        )
        print(
            f"  Max Rolling Drawdown: {rolling_metrics['rolling_drawdown'].max():.2%}"
        )
