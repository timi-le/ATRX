...
from data import DukascopyConfig, DukascopyConnector, OandaConfig, OandaConnector
...
class DualProviderManager:
    ...
    def __init__(
        self,
        oanda_config: OandaConfig | None = None,
        ...
        self.oanda_connector: OandaConnector | None = None
        self.dukascopy_connector: DukascopyConnector | None = None

        self.price_cache: dict[str, MarketData] = {}
        self.provider_status = {"oanda": False, "dukascopy": False}
    ...
    async def setup_connectors(self):
        ...
        try:
            ...
            if self.oanda_config:
                ...
                self.stream_manager.add_connector(
                    "oanda", OandaConnector, self.oanda_config
                )
            ...
            if self.dukascopy_config:
                self.dukascopy_connector = DukascopyConnector(
                    self.dukascopy_config, self.logger
                )
                self.stream_manager.add_connector(
                    "dukascopy", DukascopyConnector, self.dukascopy_config
                )
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info(
                "Dual provider system started", provider_status=self.provider_status
            )
    ...
    async def get_historical_comparison(self, symbol: str, hours: int = 1) -> dict:
        ...
        """
        self.logger.info(
            "Fetching historical data for comparison", symbol=symbol, hours=hours
        )
        ...
        try:
            ...
            if self.oanda_connector:
                ...
                self.logger.info(
                    "OANDA historical data retrieved", count=len(oanda_data)
                )
            ...
            if self.dukascopy_connector:
                ...
                results["dukascopy_data"] = {
                    ...
                    "first_candle": dukascopy_data[0].__dict__
                    if dukascopy_data
                    else None,
                    "last_candle": dukascopy_data[-1].__dict__
                    if dukascopy_data
                    else None,
                }
                self.logger.info(
                    "Dukascopy historical data retrieved", count=len(dukascopy_data)
                )
            ...
            if results["oanda_data"] and results["dukascopy_data"]:
                results["comparison"] = self._compare_historical_data(
                    results["oanda_data"], results["dukascopy_data"]
                )
        ...
        if self.dukascopy_connector:
            try:
                ...
                historical_data = await self.dukascopy_connector.get_historical_data(
                    symbol=symbol, timeframe="1h", count=5
                )

                self.logger.info(
                    "Dukascopy historical data test", count=len(historical_data)
                )
...
async def run_demo():
    ...
    dukascopy_config = DukascopyConfig(symbols=["EUR/USD", "GBP/USD"], timeout=30)
    ...
    try:
        ...
        if comparison.get("comparison"):
            ...
            if comp.get("prices_similar"):
                print(
                    f"Price similarity: {comp['prices_similar']} "
