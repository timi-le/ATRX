...
from core.position_sizer import PortfolioState, TradeSignalInput, create_position_sizer
from core.pubsub import ZMQPublisher
from core.risk_manager import RiskLevel, create_risk_manager
...
class TradingSystemWithRiskManager:
    ...
    def __init__(self):
        ...
        self.risk_manager = create_risk_manager(
            config_path="config/risk_limits.yaml", initial_capital=100000.0
        )
    ...
    async def process_signal(self, signal: Signal) -> bool:
        ...
        try:
            ...
            order = Order(
                ...
                * self.account_balance,  # Convert % to absolute
    ...
    async def _risk_monitoring_loop(self):
        ...
        while self.is_running:
            try:
                ...
                realized_pnl = sum(
                    pos.realized_pnl for pos in self.current_positions.values()
                )
                unrealized_pnl = sum(
                ...
                # Update risk manager
                await self.risk_manager.update_pnl(
...
async def main():
    ...
    try:
        ...
        for signal in signals:
            ...
            print(
                f"Signal processed: {signal.symbol} - {'SUCCESS' if success else 'REJECTED'}"
            )
