...
def create_sample_trade_data(num_trades: int = 100, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(num_trades):
        # Random trade timing
        trade_time = start_date + timedelta(hours=np.random.randint(0, 24*30))  # 30 days
    ...
    logger.info(f"Created sample trade data: {len(df)} trades, "
...
def test_monte_carlo_simulation():
    ...
    logger.info(f"✓ Monte Carlo simulation successful: {len(equity_paths)} timesteps, "
               f"{len(equity_paths.columns)} paths")
    logger.info(f"  Final equity range: [{path_finals.min():.2f}, {path_finals.max():.2f}]")
...
def test_shock_scenarios():
    ...
    for scenario in scenarios:
        ...
        logger.info(f"  {scenario.name}: {len(trades_df)} -> {len(shocked_trades)} trades, "
                   f"PnL: ${original_pnl:.2f} -> ${shocked_pnl:.2f}")
        
        # Validate shock effects
        if scenario.fill_rate_reduction > 0:
            assert len(shocked_trades) < len(trades_df), "Fill rate reduction should remove trades"
...
def test_stress_test_scenarios():
    ...
    assert 'Baseline' in results_df['scenario'].values, "Should include baseline scenario"
    
    # Check that we have expected columns
    expected_columns = ['scenario', 'total_trades', 'final_equity', 'max_drawdown', 
    ...
    for _, row in results_df.iterrows():
        logger.info(f"  {row['scenario']}: Final Equity=${row['final_equity']:.2f}, "
...
def test_comprehensive_analysis():
    ...
    assert (output_path / "monte_carlo_results.json").exists(), "Should create JSON results"
    assert (output_path / "stress_test_results.csv").exists(), "Should create CSV results"
    ...
    logger.info(f"  Baseline final equity: ${results['summary']['baseline_final_equity']:.2f}")
    logger.info(f"  Monte Carlo mean final: ${results['summary']['monte_carlo_mean_final']:.2f}")
...
def test_edge_cases():
    ...
    try:
        ...
        assert "Missing required columns" in str(e), "Should raise ValueError for missing columns"
...
def main():
    ...
    try:
        ...
        logger.info("Task 18: Monte Carlo and Stress Testing module is working correctly")
