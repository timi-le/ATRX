...
class SecureConfigLoader:
    ...
    def _detect_environment(self) -> str:
        ...
        valid_environments = ["development", "dev", "staging", "stage", "production", "prod"]
        ...
        for rule in self.validation_rules:
            try:
                ...
                if rule.min_value is not None and hasattr(value, '__lt__') and value < rule.min_value:
                    errors.append(f"Field '{rule.field_path}' value {value} is below minimum {rule.min_value}")
                
                if rule.max_value is not None and hasattr(value, '__gt__') and value > rule.max_value:
                    errors.append(f"Field '{rule.field_path}' value {value} is above maximum {rule.max_value}")
        ...
        for pattern in dangerous_patterns:
            if pattern in config_str:
                self.audit_logger.log_breach_attempt(
                    ...
                    {
                        "pattern": pattern,
                        "config_snippet": config_str[max(0, config_str.find(pattern)-20):config_str.find(pattern)+20]
                    },
                    "CRITICAL"
                )
                raise ConfigValidationError(f"Potentially malicious pattern detected in configuration: {pattern}")
    ...
    def invalidate_cache(self, config_name: Optional[str] = None) -> None:
        ...
        """
        if config_name:
            keys_to_remove = [key for key in self._config_cache.keys() if key.startswith(f"{config_name}_")]
        else:
        ...
        self.audit_logger.log_config_access(
            ...
            {
                "config_name": config_name,
                "invalidated_keys": keys_to_remove
            }
        )
