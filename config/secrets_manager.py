...
class SecretsManager:
    ...
    def __init__(self, secrets_dir: str = "config/secrets", master_key_env: str = "MASTER_ENCRYPTION_KEY"):
    ...
    def store_secret(self, scope: str, key: str, value: Union[str, Dict[str, Any]]) -> None:
        """
        ...
        if not self._check_access(scope):
            raise PermissionError(f"Access denied to scope '{scope}' for role '{self.current_role}'")
    ...
    def list_secrets(self, scope: Optional[str] = None) -> Dict[str, List[str]]:
        """
        ...
        for scope_name in scopes_to_check:
            if self._check_access(scope_name):
                result[scope_name] = list(self._secrets_vault.get(scope_name, {}).keys())
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if vault_file.exists():
                ...
                status["secrets_count"] = sum(len(scope_secrets) for scope_secrets in self._secrets_vault.values())
