...
class TLSManager:
    ...
    def setup_zmq_curve_security(self, context: zmq.Context) -> Dict[str, bytes]:
        ...
        try:
            ...
            self._zmq_authenticator.configure_curve(domain='*', location=str(self.certs_dir))
            ...
            self.audit_logger.log_security_event(
                ...
                {
                    "server_public_key_hash": hashlib.sha256(server_keys['public']).hexdigest()[:16],
                    "client_public_key_hash": hashlib.sha256(client_keys['public']).hexdigest()[:16],
    ...
    def _get_or_generate_zmq_keys(self, key_type: str) -> Dict[str, bytes]:
        ...
        try:
            ...
            public_key = self.secrets_manager.get_secret("system", f"zmq_{key_type}_public")
            secret_key = self.secrets_manager.get_secret("system", f"zmq_{key_type}_secret")
    ...
    def configure_zmq_socket_curve(self, socket: zmq.Socket, socket_type: str, role: str = "client") -> None:
        """
        ...
        try:
            if role == "server":
                ...
                server_secret = self.secrets_manager.get_secret("system", "zmq_server_secret")
                ...
                client_public = self.secrets_manager.get_secret("system", "zmq_client_public")
                client_secret = self.secrets_manager.get_secret("system", "zmq_client_secret")
                server_public = self.secrets_manager.get_secret("system", "zmq_server_public")
    ...
    def generate_self_signed_cert(self, hostname: str = "localhost", days: int = 365) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            cmd = [
                ...
                "-days", str(days),
                "-nodes",  # No passphrase
                "-subj", f"/C=US/ST=State/L=City/O=FX-Quant/OU=Trading/CN={hostname}"
            ]
            ...
            try:
                self.secrets_manager.store_secret("system", "tls_cert_path", str(cert_file))
                self.secrets_manager.store_secret("system", "tls_key_path", str(key_file))
            finally:
                self.secrets_manager.current_role = original_role
    ...
    def _generate_cert_with_cryptography(self, hostname: str, days: int) -> Tuple[str, str]:
