        ...
        if len(current_data.shape) > 1:
            ...
            for method in methods:
                if method == "psi":
                    ...
                    statistic, p_value = self.chi2_test(
...
class ModelVersionManager:
    ...
    def get_best_version(
        self, metric: str = "validation_score"
    ) -> ModelVersion | None:
        """
...
class RetrainingScheduler:
    ...
    def should_retrain(
        ...
        if (
            ...
            and datetime.now() - self.last_training_time < timedelta(days=min_days)
            and not force_check
        ):
            return False, triggers
    ...
    def _is_scheduled_time(self, frequency: str) -> bool:
        ...
        if frequency == "hourly":
            ...
            return (
                ...
                and now.minute < scheduled_minute + 5
            )  # 5-minute window
        elif frequency == "weekly":
            return (
                now.weekday() == 0
                and now.hour == scheduled_hour  # Monday
                and now.minute >= scheduled_minute
                and now.minute < scheduled_minute + 5
            )
            ...
            return (
                now.day == 1
                and now.hour == scheduled_hour  # First day of month
                and now.minute >= scheduled_minute
                and now.minute < scheduled_minute + 5
            )
    ...
    async def retrain_models(
        ...
        try:
            ...
            results = self.pipeline.run_full_training_pipeline(
                X=X, y=y, models_to_train=models_to_train, optimize_hyperparameters=True
            )
            ...
            for result in results:
                if self._validate_new_model(result):
                    ...
                    version = self.version_manager.add_version(
                        ...
                        validation_score=result.metrics.get(
    ...
    def _hash_data(self, X: np.ndarray, y: np.ndarray) -> str:
        ...
        data_str = (
            f"{X.shape}_{y.shape}_{np.mean(X)}_{np.std(X)}_{np.mean(y)}_{np.std(y)}"
        )
    ...
    def start_monitoring(self, check_interval: int = 3600) -> None:
        ...
        """
        self.logger.info(
            "Starting retraining monitoring", check_interval=check_interval
        )
