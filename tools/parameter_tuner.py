...
class ParameterTuner:
    ...
    VALIDATION_RULES = {
        ...
        "grid_step_factor": {"type": float, "min": 0.1, "max": 2.0, "description": "Grid step multiplier"},
        "martingale_multiplier": {"type": float, "min": 1.0, "max": 5.0, "description": "Martingale position multiplier"},
        "max_levels": {"type": int, "min": 1, "max": 10, "description": "Maximum grid levels"},
        "breakout_zone_buffer": {"type": float, "min": 0.0001, "max": 0.01, "description": "Breakout zone buffer in price units"},
        
        # Risk Management Parameters
        "risk_per_trade": {"type": float, "min": 0.001, "max": 0.1, "description": "Risk percentage per trade"},
        "max_drawdown": {"type": float, "min": 0.01, "max": 0.5, "description": "Maximum allowed drawdown"},
        "max_positions": {"type": int, "min": 1, "max": 50, "description": "Maximum concurrent positions"},
        "position_size_limit": {"type": float, "min": 0.01, "max": 10.0, "description": "Maximum position size"},
        
        # Volatility and Market Filters
        "volatility_filter_threshold": {"type": float, "min": 0.0, "max": 1.0, "description": "Volatility filter threshold"},
        "trend_filter_strength": {"type": float, "min": 0.0, "max": 1.0, "description": "Trend filter strength"},
        "session_filter_enabled": {"type": bool, "description": "Enable session-based filtering"},
        "spread_filter_max": {"type": float, "min": 0.0, "max": 0.01, "description": "Maximum allowed spread"},
        
        # Take Profit / Stop Loss
        "tp_type": {"type": str, "allowed_values": ["fixed", "dynamic", "trailing"], "description": "Take profit type"},
        "sl_type": {"type": str, "allowed_values": ["fixed", "dynamic", "trailing"], "description": "Stop loss type"},
        "tp_ratio": {"type": float, "min": 0.5, "max": 10.0, "description": "Take profit ratio"},
        "sl_ratio": {"type": float, "min": 0.1, "max": 5.0, "description": "Stop loss ratio"},
        
        # Strategy Weights
        "momentum_weight": {"type": float, "min": 0.0, "max": 1.0, "description": "Momentum strategy weight"},
        "mean_reversion_weight": {"type": float, "min": 0.0, "max": 1.0, "description": "Mean reversion strategy weight"},
        "breakout_weight": {"type": float, "min": 0.0, "max": 1.0, "description": "Breakout strategy weight"},
        "regime_adaptive_weight": {"type": float, "min": 0.0, "max": 1.0, "description": "Regime adaptive weight"},
        
        # Technical Indicator Parameters
        "rsi_period": {"type": int, "min": 5, "max": 50, "description": "RSI calculation period"},
        "ma_fast_period": {"type": int, "min": 5, "max": 50, "description": "Fast moving average period"},
        "ma_slow_period": {"type": int, "min": 10, "max": 200, "description": "Slow moving average period"},
        "bollinger_period": {"type": int, "min": 10, "max": 50, "description": "Bollinger bands period"},
        "bollinger_std": {"type": float, "min": 1.0, "max": 3.0, "description": "Bollinger bands standard deviation"},
    }
    ...
    def get_parameter(self, key: str) -> Any:
        ...
        try:
            ...
            raise ParameterValidationError(f"Parameter '{key}' not found in configuration")
    ...
    def save_config(self, versioned: bool = True) -> str:
        ...
        if self.changes_made:
            ...
            for change in self.changes_made:
                self.logger.info(f"  {change['parameter']}: {change['old_value']} -> {change['new_value']}")
    ...
    def suggest_optimization(self, param_name: str) -> Dict[str, Any]:
        ...
        if param_name in self.VALIDATION_RULES:
            ...
            if rule["type"] in [int, float]:
                ...
                suggestion["reasoning"] = f"Valid range based on system constraints: {min_val} to {max_val}"
                ...
                if current_value is not None:
                    ...
                    if current_value <= min_val * 1.1:
                        ...
                        suggestion["reasoning"] += f". Current value ({current_value}) is near minimum, consider increasing."
                    elif current_value >= max_val * 0.9:
                        suggestion["suggested_value"] = mid_point
                        suggestion["reasoning"] += f". Current value ({current_value}) is near maximum, consider decreasing."
            ...
            suggestion["reasoning"] = "No optimization rules available for this parameter"
...
Examples:
  ...
  %(prog)s --validate-all
    ...
    parser.add_argument("--set", action="append", dest="set_params",
                       help="Set parameter (key=value format)")
    parser.add_argument("--get", dest="get_param",
                       help="Get parameter value")
    parser.add_argument("--info", dest="param_info",
                       help="Get parameter information")
    parser.add_argument("--suggest", dest="suggest_param",
    ...
    parser.add_argument("--restore", dest="restore_version",
                       help="Restore configuration from version (timestamp)")
    parser.add_argument("--validate-all", action="store_true",
                       help="Validate all parameters")
    ...
    try:
        ...
        if args.param_info:
            ...
            if info['validation_rule']:
                ...
                if 'allowed_values' in rule: print(f"  Allowed: {rule['allowed_values']}")
