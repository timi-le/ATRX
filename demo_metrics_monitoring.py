...
class TradingSessionSimulator:
    ...
    def simulate_trade_execution(self, symbol: str, side: str, quantity: int = 10000):
        ...
        # Log trade result
        if will_fill:
            logger.info(f"✅ {side} {symbol}: ${pnl:+.2f} PnL, "
                       f"{execution_time*1000:.1f}ms, {slippage_pips:.1f} pips slippage")
        else:
            logger.info(f"❌ {side} {symbol}: REJECTED after {execution_time*1000:.1f}ms")
    ...
    def run_trading_session(self, duration_minutes: int = 10, trades_per_minute: float = 2.0):
        ...
        while time.time() < end_time and self.running:
            ...
            if time.time() - last_update > 5:  # Every 5 seconds
                ...
                logger.info(f"📊 Session update ({elapsed_minutes:.1f}min): "
    ...
    def _print_session_summary(self):
        ...
        logger.info("=" * 40)
        logger.info(f"Duration: {(datetime.now() - self.start_time).total_seconds()/60:.1f} minutes")
        logger.info(f"Total Trades: {self.total_trades}")
        logger.info(f"Winning Trades: {self.winning_trades}")
        logger.info(f"Win Rate: {(self.winning_trades/self.total_trades)*100 if self.total_trades > 0 else 0:.1f}%")
