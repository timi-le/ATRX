...
class BulkParameterUpdate(BaseModel):
    ...
    parameters: dict[str, Any] = Field(
        ..., description="Dictionary of parameter updates"
    )
...
async def health_check():
    ...
    try:
        ...
        return ApiResponse(success=False, message=f"Health check failed: {str(e)}")
...
async def get_all_parameters():
    ...
    try:
        ...
        return ApiResponse(
            success=True, message="Parameters retrieved successfully", data=parameters
        )
    except Exception as e:
        raise HTTPException(
...
async def get_parameter(param_name: str):
    ...
    try:
        ...
        return ApiResponse(
            ...
            data={"name": param_name, "value": value, "info": info},
        )
    except Exception as e:
...
async def update_parameter(update: ParameterUpdate):
    ...
    """
    try:
        success = tuner.set_parameter(
            update.key, update.value, validate=update.validate
        )
    ...
    try:
        ...
        for key, value in update.parameters.items():
            try:
                ...
                results[key] = {"success": success, "new_value": value}
            except Exception as e:
                errors[key] = str(e)
    ...
    try:
        ...
        if success:
            return ApiResponse(
                ...
                data={"restored_timestamp": timestamp, "current_config": tuner.config},
            )
        else:
            raise HTTPException(
                status_code=404, detail=f"Version {timestamp} not found"
            )
...
async def not_found_handler(request, exc):
    return JSONResponse(
        status_code=404,
        content=ApiResponse(success=False, message="Endpoint not found").dict(),
    )
