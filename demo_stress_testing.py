...
def load_backtest_results(results_file: str = "outputs/backtest_results.csv") -> pd.DataFrame:
    """
    ...
    try:
        ...
        if Path(results_file).exists():
            ...
            logger.warning(f"Backtest results file {results_file} not found. Generating sample data.")
...
def generate_realistic_sample_data(num_trades: int = 200) -> pd.DataFrame:
    ...
    for i in range(num_trades):
        ...
        # Realistic lot sizes
        quantity = np.random.choice([10000, 25000, 50000, 100000], 
                                   p=[0.4, 0.3, 0.2, 0.1])
        ...
        if np.random.random() < 0.58:  # 58% win rate
            pnl = abs(np.random.lognormal(3.5, 0.8))  # Positive PnL, log-normal distribution
        else:  # Losing trade
...
def main():
    ...
    print(f"Probability of loss: {mc_analysis['risk_metrics']['probability_of_loss']:.1%}")
    print(f"Average max drawdown: {mc_analysis['risk_metrics']['avg_max_drawdown']:.1%}")
    ...
    for _, row in stress_results_sorted.iterrows():
        ...
        baseline_equity = stress_results[stress_results['scenario'] == 'Baseline']['final_equity'].iloc[0]
        equity_change = ((row['final_equity'] - baseline_equity) / baseline_equity) * 100
    ...
    comprehensive_results = run_comprehensive_stress_test(trades_df, output_dir)
    ...
    high_slippage_impact = stress_results[stress_results['scenario'] == 'High Slippage']['final_equity'].iloc[0]
    baseline_equity = stress_results[stress_results['scenario'] == 'Baseline']['final_equity'].iloc[0]
    slippage_sensitivity = abs((high_slippage_impact - baseline_equity) / baseline_equity)
