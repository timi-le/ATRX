...
try:
    from monitoring.metrics_server import get_metrics, start_metrics_server
except ImportError:
    print(
        "⚠️  Warning: Could not import metrics server. Please ensure Task 19 is completed."
    )
...
class GrafanaTestSuite:
    ...
    def test_docker_containers(self):
        ...
        required_containers = ["fx-prometheus", "fx-grafana", "fx-alertmanager"]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_metrics:
                    logger.info(
                        "✓ Metrics server is accessible and serving expected metrics"
                    )
                    return True
                else:
        ...
        try:
            ...
            if query_response.status_code == 200:
                ...
                if data.get("data", {}).get("result"):
                    ...
                    logger.warning(
                        "⚠️  Prometheus connected but no FX metrics data found"
                    )
                    return True
            else:
    ...
    def test_grafana_connectivity(self):
        ...
        try:
            ...
            if auth_response.status_code == 200:
                ...
                logger.info(
                    f"✓ Grafana authentication successful (user: {user_info.get('login', 'unknown')})"
                )
                return True
            else:
                logger.error(
                    f"❌ Grafana authentication failed: {auth_response.status_code}"
                )
                return False
    ...
    def test_grafana_datasource(self):
        ...
        try:
            ...
            logger.info(
                f"✓ Prometheus datasource found: {prometheus_ds.get('name', 'Unknown')}"
            )
            ...
            if health_response.status_code == 200:
                ...
                logger.error(
                    f"❌ Datasource health check request failed: {health_response.status_code}"
                )
                return False
    ...
    def test_grafana_dashboards(self):
        ...
        try:
            ...
            if len(found_dashboards) >= 2:
                ...
                logger.warning(
                    f"⚠️  Expected dashboards not found. Found: {found_dashboards}"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                api_response = requests.get(
                    f"{self.alertmanager_url}/api/v1/alerts", timeout=5
                )
                if api_response.status_code == 200:
                    ...
                    logger.warning(
                        "⚠️  Alertmanager API not accessible, but service is healthy"
                    )
                    return True
            else:
                logger.error(
    ...
    def test_sample_metrics_generation(self):
        ...
        if not start_metrics_server or not get_metrics:
            logger.warning(
                "⚠️  Metrics server not available, skipping sample data generation"
            )
            return True
    ...
    def run_comprehensive_test(self):
        ...
        if passed == total:
            ...
            logger.info(
                "  • Docker containers not running: Run 'docker-compose -f docker-compose.monitoring.yml up -d'"
            )
            logger.info(
                "  • Metrics server not accessible: Start with 'python demo_metrics_monitoring.py'"
            )
            logger.info(
                "  • Dashboards not found: Import manually from monitoring/grafana/dashboards/"
            )
