...
class BreakoutTrendStrategy(Strategy):
    ...
    def __init__(
        ...
        # Strategy parameters
        self.entry_patterns = config.get(
            "entry_patterns", ["inside_bar", "engulfing", "break_retest"]
        )
        ...
        self.retest_confirmation_required = config.get(
            "retest_confirmation_required", True
        )
    ...
    def _detect_price_patterns(
        self, symbol: str, features: dict[str, float]
    ) -> tuple[PatternType, OrderSide, float] | None:
    ...
    def _check_retest_completion(
        self, symbol: str, level: BreakoutLevel, side: OrderSide
    ) -> bool:
    ...
    def _check_trend_alignment(
        self, symbol: str, features: dict[str, float], side: OrderSide
    ) -> bool:
    ...
    async def _manage_existing_position(
        ...
        # Check trailing stop
        if position.trailing_stop and self._should_trailing_stop(
    ...
    def _update_trailing_stop(
        self, position: TrendPosition, current_price: float, features: dict[str, float]
    ) -> None:
    ...
    def _should_trailing_stop(
        self, position: TrendPosition, current_price: float
    ) -> bool:
