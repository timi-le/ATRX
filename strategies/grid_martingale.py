...
class GridMartingaleStrategy(Strategy):
    ...
    def __init__(
        ...
        self.grid_spacing_atr_multiplier = config.get('grid_spacing_atr_multiplier', 0.5)
    ...
    async def generate_signal(
        ...
        try:
            ...
            if entry_signal:
                # Initialize new grid
                await self._initialize_grid(symbol, current_price, features, entry_signal.side)
                return entry_signal
    ...
    async def _check_grid_entry(
        ...
        if side == OrderSide.BUY:
            ...
            strength = max(0.1, (rsi - self.rsi_overbought) / (100 - self.rsi_overbought))
    ...
    async def _initialize_grid(
        ...
        if self.entry_mode == 'bidirectional':
            ...
            grid_levels.extend(self._create_grid_levels(
    ...
    async def _close_grid(self, symbol: str, reason: str) -> Signal:
        ...
        net_long_size = sum(level.size for level in grid.grid_levels if level.side == OrderSide.BUY)
        net_short_size = sum(level.size for level in grid.grid_levels if level.side == OrderSide.SELL)
