...
class TimeScalpingStrategy(Strategy):
    ...
    def __init__(
        ...
        self.active_sessions = [TradingSession(s) for s in config.get('active_sessions', ['london', 'tokyo'])]
        self.patterns = [ScalpingPattern(p) for p in config.get('patterns', ['double_top', 'double_bottom', 'flag', 'pennant'])]
        ...
        self.session_trade_count: Dict[TradingSession, int] = {session: 0 for session in TradingSession}
        self.price_history: Dict[str, List[Tuple[datetime, float, float, float, float]]] = {}
        ...
        # Session definitions
        self.sessions = {
            TradingSession.LONDON: SessionInfo(TradingSession.LONDON, time(8, 0), time(17, 0)),
            TradingSession.TOKYO: SessionInfo(TradingSession.TOKYO, time(0, 0), time(9, 0)),
            TradingSession.NEW_YORK: SessionInfo(TradingSession.NEW_YORK, time(13, 0), time(22, 0)),
            TradingSession.SYDNEY: SessionInfo(TradingSession.SYDNEY, time(22, 0), time(7, 0))
        }
    ...
    async def generate_signal(
        ...
        try:
            ...
            entry_signal = await self._check_scalping_entry(symbol, features, current_session)
    ...
    def _get_current_session(self) -> Optional[TradingSession]:
    ...
    async def _check_scalping_entry(
        ...
        confidence = self._calculate_scalping_confidence(symbol, features, pattern_type, strength)
    ...
    def _check_rsi_reversal(self, symbol: str, features: Dict[str, float]) -> Optional[Tuple[OrderSide, float]]:
        ...
        if rsi <= (50 - self.rsi_entry_threshold):
            strength = max(0.1, (50 - self.rsi_entry_threshold - rsi) / (50 - self.rsi_entry_threshold))
            return OrderSide.BUY, strength
        
        # Check for overbought reversal (sell signal)
        elif rsi >= (50 + self.rsi_entry_threshold):
            strength = max(0.1, (rsi - (50 + self.rsi_entry_threshold)) / (50 - self.rsi_entry_threshold))
    ...
    def _detect_scalping_patterns(
    ...
    ) -> Optional[Tuple[ScalpingPattern, float]]:
        ...
        if ScalpingPattern.DOUBLE_TOP in self.patterns or ScalpingPattern.DOUBLE_BOTTOM in self.patterns:
    ...
    def _detect_double_patterns(
        ...
        if expected_side == OrderSide.SELL and ScalpingPattern.DOUBLE_TOP in self.patterns:
            max_high = max(recent_highs)
            high_indices = [i for i, h in enumerate(recent_highs) if abs(h - max_high) < max_high * 0.001]
        ...
        if expected_side == OrderSide.BUY and ScalpingPattern.DOUBLE_BOTTOM in self.patterns:
            min_low = min(recent_lows)
            low_indices = [i for i, l in enumerate(recent_lows) if abs(l - min_low) < min_low * 0.001]
    ...
    def _detect_flag_pattern(
        ...
        if recent_range < earlier_range * 0.5:  # Consolidation detected
            ...
            move_strength = abs(recent_closes[-10] - recent_closes[-15]) / recent_closes[-15]
    ...
    def _should_stop_loss(self, position: ScalpingPosition, current_price: float) -> bool:
    ...
    def _should_take_profit(self, position: ScalpingPosition, current_price: float) -> bool:
    ...
    def get_session_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'session_trade_counts': {s.value: count for s, count in self.session_trade_count.items()},
            'max_trades_per_session': self.max_trades_per_session,
            'active_sessions': [s.value for s in self.active_sessions]
        } 