...
async def test_performance_tracking():
    ...
    print(f"✓ PerformanceAnalyzer created, initial equity: ${performance_analyzer.current_equity:.2f}")
    ...
    if len(performance_analyzer.trades) > 0:
        ...
        print(f"   ✓ Trade created: {trade.symbol} {trade.side.value} P&L=${trade.pnl:.2f}")
    else:
    ...
    print(f"1. Recording SELL SHORT fill: {sell_short_fill.quantity} @ {sell_short_fill.price}")
    ...
    print(f"2. Recording BUY COVER fill: {buy_cover_fill.quantity} @ {buy_cover_fill.price}")
    ...
    fills_and_positions = [
        # Random BUY
        (Fill("order1", "EURUSD", OrderSide.BUY, 100000, 1.1000, datetime.now(), 7.0, 0.0001, FillType.FULL),
         {"EURUSD": Position("EURUSD", 100000, 1.1000, 0.0)}),
        
        # Another random BUY (add to position)
        (Fill("order2", "EURUSD", OrderSide.BUY, 100000, 1.1010, datetime.now(), 7.0, 0.0001, FillType.FULL),
         {"EURUSD": Position("EURUSD", 200000, 1.1005, 0.0)}),
        
        # Random SELL on different symbol
        (Fill("order3", "GBPUSD", OrderSide.SELL, 100000, 1.3000, datetime.now(), 7.0, 0.0001, FillType.FULL),
         {"EURUSD": Position("EURUSD", 200000, 1.1005, 0.0), "GBPUSD": Position("GBPUSD", -100000, 1.3000, 0.0)}),
        
        # Random BUY on different symbol
        (Fill("order4", "USDJPY", OrderSide.BUY, 100000, 145.00, datetime.now(), 7.0, 0.0001, FillType.FULL),
         {"EURUSD": Position("EURUSD", 200000, 1.1005, 0.0), "GBPUSD": Position("GBPUSD", -100000, 1.3000, 0.0), "USDJPY": Position("USDJPY", 100000, 145.00, 0.0)}),
    ]
    
    for i, (fill, positions) in enumerate(fills_and_positions, 1):
        print(f"{i}. Recording {fill.side.value} fill: {fill.symbol} {fill.quantity} @ {fill.price}")
