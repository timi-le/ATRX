...
from config.tls_manager import TLSManager
...
class SecurityTestSuite:
    ...
    def __init__(self):
        ...
        self.secrets_manager = SecretsManager(secrets_dir=str(self.secrets_dir))
        self.config_loader = SecureConfigLoader(config_dir=str(self.config_dir))
        self.audit_logger = AuditLogger(log_dir=str(self.logs_dir))
        self.tls_manager = TLSManager(certs_dir=str(self.certs_dir))
        ...
        for scope in restricted_scopes:
            try:
                self.secrets_manager.store_secret(
                    scope, "service_test", "service_value"
                )
    ...
    def _test_env_overrides(self) -> bool:
        ...
        # Create base config
        base_config = {"trading": {"environment": "paper", "risk_per_trade": 0.02}}

        # Create production override
        prod_config = {"trading": {"environment": "live", "risk_per_trade": 0.01}}
        ...
        try:
            ...
            return (
                loaded_config["trading"]["environment"] == "live"
                and loaded_config["trading"]["risk_per_trade"] == 0.01
            )
    ...
    def _test_security_checks(self) -> bool:
        ...
        malicious_config = {
            "database": {"command": "eval(dangerous_code)", "host": "localhost"}
        }
    ...
    def _test_security_logging(self) -> bool:
        ...
        self.audit_logger.log_security_event(
            "TEST_SECURITY_EVENT", {"test_data": "security_test"}
        )
    ...
    def _test_config_logging(self) -> bool:
        ...
        self.audit_logger.log_config_access(
            "TEST_CONFIG_ACCESS", {"config_name": "test_config"}
        )
    ...
    def _test_breach_detection(self) -> bool:
        ...
        for _ in range(6):  # Exceed threshold of 5
            self.audit_logger.log_security_event(
                "SECRETS_LOAD_FAILED", {"error": "decryption_failed"}
            )
        ...
        breach_events = [
            e for e in events if e.get("breach_type") == "MULTIPLE_FAILED_DECRYPTION"
        ]
    ...
    def _test_e2e_secret_flow(self) -> bool:
        ...
        config_data = {
            "api": {"key": "secret://api/api_key", "url": "https://api.example.com"},
            "database": {"password": "secret://database/password", "host": "localhost"},
        }
        ...
        return (
            loaded_config["api"]["key"] == "test_api_key_value"
            and loaded_config["database"]["password"] == "test_db_password"
        )
    ...
    def _test_config_with_secrets(self) -> bool:
        ...
        config_data = {
            "trading": {
                ...
                "max_positions": 10,
                "environment": "paper",
                "broker": {"token": "secret://api/oanda_token"},
            },
            "database": {"host": "localhost", "port": 5432},
        }
    ...
    def _test_security_monitoring(self) -> bool:
        ...
        self.audit_logger.log_security_event(
            "SYSTEM_START", {"component": "trading_engine"}
        )
        self.audit_logger.log_config_access("CONFIG_LOADED", {"config": "main"})
        self.audit_logger.log_security_event(
            "ENCRYPTION_KEY_DERIVED", {"algorithm": "PBKDF2"}
        )
        ...
        return (
            ...
            and "event_type_counts" in report
            and "access_type_counts" in report
        )
