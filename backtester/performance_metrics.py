...
from core.interfaces.trading_interfaces import Position, OrderSide
...
class PerformanceConfig:
    ...
    # Regime analysis
    regime_labels: List[str] = field(default_factory=lambda: ["trending", "ranging", "volatile"])
...
class PerformanceAnalyzer:
    ...
    def __init__(self, config: PerformanceConfig, logger: Optional[structlog.stdlib.BoundLogger] = None):
        self.config = config
        ...
        self.logger.info("PerformanceAnalyzer initialized", initial_capital=config.initial_capital)
    ...
    def update_equity(self, timestamp: datetime, new_equity: float) -> None:
        ...
        if new_equity > self.peak_equity:
            ...
            if self.drawdown_start:
                duration = (timestamp - self.drawdown_start).total_seconds() / 3600  # hours
    ...
    def update_positions(self, timestamp: datetime, positions: Dict[str, Position]) -> None:
    ...
    def record_fill(self, fill: Fill, current_positions: Dict[str, Position]) -> None:
        ...
        symbol = fill.symbol
        self.logger.debug("record_fill called", fill=fill, current_positions_from_exec_sim=current_positions, internal_tracked_positions_before=getattr(self, '_internal_positions', {}).copy())
        ...
        self.logger.debug("Got previous_tracked_position", previous_tracked_position=previous_tracked_position)
        ...
        self.logger.debug("Got current_sim_position from ExecutionSimulator", current_sim_position=current_sim_position)
        ...
        if fill.side == OrderSide.BUY:
            ...
            self._handle_sell_fill(fill, previous_tracked_position, current_sim_position)
        ...
        self.logger.debug("record_fill finished", internal_tracked_positions_after=getattr(self, '_internal_positions', {}).copy(), current_equity=self.current_equity)
        ...
        if self.fill_counter % 1000 == 0:
            self.logger.info(f"Processed {self.fill_counter} fills -> {len(self.trades)} completed trades, equity: ${self.current_equity:.2f}")

    def _get_tracked_position(self, symbol: str) -> Optional[Dict]:
        """Get our internally tracked position for a symbol."""
        pos = getattr(self, '_internal_positions', {}).get(symbol)
        self.logger.debug("_get_tracked_position called", symbol=symbol, position_found=pos)
        return pos
    
    def _update_tracked_position(self, symbol: str, current_sim_position: Optional[Position], timestamp: datetime) -> None:
        ...
        self.logger.debug("_update_tracked_position called", symbol=symbol, current_sim_position=current_sim_position, timestamp=timestamp, internal_pos_before_update=self._internal_positions.get(symbol))

        existing_tracked_pos = self._internal_positions.get(symbol)

        if current_sim_position and abs(current_sim_position.quantity) > 1e-9: # Use epsilon for float comparison
            if not existing_tracked_pos or abs(existing_tracked_pos.get('quantity', 0)) < 1e-9: # Was new or zero
                entry_time = timestamp # This fill opened/reopened the position
                self.logger.debug("Position is new or was zero. Setting new entry_time", symbol=symbol, entry_time=entry_time)
            else:
                entry_time = existing_tracked_pos.get('entry_time', timestamp) # Preserve existing entry time
                self.logger.debug("Position exists. Preserving entry_time", symbol=symbol, entry_time=entry_time)
            ...
            self.logger.debug("Updated/set internal position", symbol=symbol, new_tracked_pos=self._internal_positions[symbol])
        else:
            # Position is closed (or effectively zero), remove from tracking
            if symbol in self._internal_positions:
                self.logger.debug("Position closed or zero. Removing from internal tracking", symbol=symbol, last_tracked_pos=self._internal_positions[symbol])
                self._internal_positions.pop(symbol, None)
            else:
                self.logger.debug("Position closed or zero, and was not in internal tracking.", symbol=symbol)

    def _handle_buy_fill(self, fill: Fill, previous_tracked_position: Optional[Dict], current_sim_position: Optional[Position]) -> None:
        """Handle a BUY fill - opening long or closing short."""
        symbol = fill.symbol
        self.logger.debug("_handle_buy_fill called", fill=fill, previous_tracked_position=previous_tracked_position, current_sim_position=current_sim_position)
        
        # Check if we had a short position (in our internal tracking) that's being closed by this BUY fill
        if previous_tracked_position and previous_tracked_position.get('quantity', 0) < -1e-9: # Epsilon for float comparison
            self.logger.debug("Identified closing of a short position", symbol=symbol, prev_qty=previous_tracked_position['quantity'], fill_qty=fill.quantity)
            ...
            if close_quantity > 1e-9: # Ensure meaningful quantity is being closed
                self.logger.debug("Calculating P&L for closed short portion", symbol=symbol, close_qty=close_quantity, prev_avg_price=previous_tracked_position['avg_price'], fill_price=fill.price)
                pnl = close_quantity * (previous_tracked_position['avg_price'] - fill.price)
                pnl -= fill.commission
                self.logger.debug("P&L calculated for short close", pnl=pnl, commission=fill.commission)
                ...
                self._create_completed_trade(
                    ...
                    entry_time=previous_tracked_position.get('entry_time', fill.timestamp),
                    exit_time=fill.timestamp,
                    pnl=pnl,
                    commission=fill.commission
                )
            else:
                self.logger.debug("Close quantity too small, not creating trade for short close", symbol=symbol, close_qty=close_quantity)
        else:
            self.logger.debug("BUY fill did not close a previous short position or no previous short position existed.", symbol=symbol, prev_pos=previous_tracked_position)
        

    def _handle_sell_fill(self, fill: Fill, previous_tracked_position: Optional[Dict], current_sim_position: Optional[Position]) -> None:
        """Handle a SELL fill - closing long or opening short."""
        symbol = fill.symbol
        self.logger.debug("_handle_sell_fill called", fill=fill, previous_tracked_position=previous_tracked_position, current_sim_position=current_sim_position)
        
        # Check if we had a long position (in our internal tracking) that's being closed by this SELL fill
        if previous_tracked_position and previous_tracked_position.get('quantity', 0) > 1e-9: # Epsilon for float comparison
            self.logger.debug("Identified closing of a long position", symbol=symbol, prev_qty=previous_tracked_position['quantity'], fill_qty=fill.quantity)
            prev_long_quantity = previous_tracked_position['quantity']
            close_quantity = min(fill.quantity, prev_long_quantity)
            
            if close_quantity > 1e-9: # Ensure meaningful quantity is being closed
                self.logger.debug("Calculating P&L for closed long portion", symbol=symbol, close_qty=close_quantity, prev_avg_price=previous_tracked_position['avg_price'], fill_price=fill.price)
                pnl = close_quantity * (fill.price - previous_tracked_position['avg_price'])
                pnl -= fill.commission
                self.logger.debug("P&L calculated for long close", pnl=pnl, commission=fill.commission)
                ...
                self.logger.debug("Close quantity too small, not creating trade for long close", symbol=symbol, close_qty=close_quantity)
        else:
            self.logger.debug("SELL fill did not close a previous long position or no previous long position existed.", symbol=symbol, prev_pos=previous_tracked_position)

    def _create_completed_trade(self, symbol: str, side: OrderSide, entry_price: float, 
                              exit_price: float, quantity: float, entry_time: datetime, 
                              exit_time: datetime, pnl: float, commission: float) -> None:
    ...
    def _update_drawdown_tracking(self, timestamp: datetime) -> None:
        ...
        if self.current_equity > self.peak_equity:
            ...
            if self.drawdown_start:
                ...
                duration = (timestamp - self.drawdown_start).total_seconds() / 86400  # days
    ...
    def calculate_metrics(self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> PeriodMetrics:
        ...
        if returns_data:
            ...
            downside_deviation = np.std(negative_returns) * np.sqrt(self.config.trading_days_per_year) if negative_returns else 0.0
    ...
    def _calculate_trade_statistics(self, trades: List[TradeMetrics]) -> Dict[str, float]:
    ...
    def calculate_regime_performance(self) -> Dict[str, Dict[str, float]]:
        ...
        for regime, returns in self.regime_performance.items():
            ...
            # Sharpe ratio
            excess_return = np.mean(returns) * self.config.trading_days_per_year - self.config.risk_free_rate
            sharpe_ratio = excess_return / volatility if volatility > 0 else 0.0
    ...
    def calculate_rolling_metrics(self, window_days: int = 30) -> pd.DataFrame:
        ...
        rolling_return = df['return'].rolling(window=window_days).mean() * self.config.trading_days_per_year
        rolling_volatility = df['return'].rolling(window=window_days).std() * np.sqrt(self.config.trading_days_per_year)
        rolling_sharpe = (rolling_return - self.config.risk_free_rate) / rolling_volatility
        
        # Rolling drawdown
        cumulative_returns = (1 + df['return']).cumprod()
        rolling_max = cumulative_returns.rolling(window=window_days, min_periods=1).max()
    ...
    def calculate_expected_shortfall(self, confidence_level: Optional[float] = None) -> float:
    ...
    def _filter_equity_data(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> List[Tuple[datetime, float]]:
    ...
    def _filter_returns_data(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> List[Tuple[datetime, float]]:
    ...
    def _filter_trades_data(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> List[TradeMetrics]:
    ...
    def _empty_metrics(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> PeriodMetrics:
