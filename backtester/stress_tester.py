...
class MonteCarloStressTester:
    ...
    def __init__(self, config: StressTestConfig = None):
        ...
        logger.info(f"MonteCarloStressTester initialized with {self.config.num_monte_carlo_paths} paths")
    ...
    def simulate_equity_paths(self, 
        ...
        logger.info(f"Generating {num_paths} Monte Carlo equity paths from {len(trades_df)} trades")
        ...
        for path_idx in range(num_paths):
            ...
            resampled_indices = np.random.choice(len(trade_returns), 
        ...
        logger.info(f"Generated Monte Carlo paths with final equity range: "
    ...
    def apply_shocks(self, 
                    trades_df: pd.DataFrame, 
                    scenario: ShockScenario) -> pd.DataFrame:
        """
        ...
        if 'slippage' in shocked_trades.columns and scenario.slippage_multiplier != 1.0:
            ...
            logger.debug(f"Slippage shock: {original_slippage:.6f} -> {new_slippage:.6f}")
        
        # Apply commission shock
        if 'commission' in shocked_trades.columns and scenario.commission_multiplier != 1.0:
            ...
            logger.debug(f"Commission shock: {original_commission:.2f} -> {new_commission:.2f}")
        ...
        if scenario.fill_rate_reduction > 0:
            ...
            if num_to_remove > 0:
                indices_to_remove = np.random.choice(shocked_trades.index, 
                                                   size=num_to_remove, 
                                                   replace=False)
                shocked_trades = shocked_trades.drop(indices_to_remove)
                logger.debug(f"Fill rate shock: Removed {num_to_remove} trades "
        ...
        if 'slippage' in shocked_trades.columns or 'commission' in shocked_trades.columns:
            # Calculate additional costs from shocks
            original_slippage_cost = 0
            original_commission_cost = 0
            ...
            if 'slippage' in shocked_trades.columns:
                ...
                slippage_cost_adjustment = (shocked_trades['slippage'] * 
        ...
        if scenario.volatility_multiplier != 1.0 and 'price' in shocked_trades.columns:
            ...
            logger.debug(f"Volatility shock applied: {scenario.volatility_multiplier}x multiplier")
        
        logger.info(f"Shock scenario '{scenario.name}' applied. "
    ...
    def calculate_var_cvar(self, 
        ...
        logger.debug(f"VaR/CVaR calculated: VaR({confidence:.1%}) = {var:.4f}, "
    ...
    def run_stress_tests(self, 
                        trades_df: pd.DataFrame, 
                        scenarios: List[ShockScenario]) -> pd.DataFrame:
        """
        ...
        baseline_metrics = self._calculate_performance_metrics(trades_df, baseline_equity)
        ...
        for scenario in scenarios:
            try:
                ...
                if len(shocked_trades) == 0:
                    logger.warning(f"No trades remaining after shock scenario: {scenario.name}")
                    continue
                ...
                metrics = self._calculate_performance_metrics(shocked_trades, shocked_equity)
    ...
    def _calculate_performance_metrics(self, 
        ...
        total_return = (final_equity - initial_equity) / abs(initial_equity) if initial_equity != 0 else 0
    ...
    def generate_monte_carlo_report(self, 
                                  ...
                                  output_dir: str = "outputs/stress_testing") -> Dict[str, Any]:
        """
        ...
        percentile_values = {f"p{p}": np.percentile(final_values, p) for p in percentiles}
    ...
    def plot_monte_carlo_paths(self, 
        ...
        plt.fill_between(equity_paths_df.index, lower_5, upper_95, 
        ...
        if show_individual_paths:
            ...
            sample_columns = np.random.choice(equity_paths_df.columns, 
                                            size=paths_to_plot, 
                                            replace=False)
            
            for col in sample_columns:
                plt.plot(equity_paths_df.index, equity_paths_df[col], 
                        alpha=0.1, color='gray', linewidth=0.5)
        
        # Plot key statistics
        plt.plot(equity_paths_df.index, median, 
                color='blue', linewidth=2, label='Median Path')
        plt.plot(equity_paths_df.index, mean_path, 
                color='red', linewidth=2, label='Mean Path')
        
        # Formatting
        plt.title(f'Monte Carlo Equity Paths ({len(equity_paths_df.columns)} simulations)')
        plt.xlabel('Time')
        plt.ylabel('Equity')
        plt.legend()
...
def run_comprehensive_stress_test(trades_df: pd.DataFrame, 
                                output_dir: str = "outputs/stress_testing") -> Dict[str, Any]:
    """
    ...
    plot_path = stress_tester.plot_monte_carlo_paths(equity_paths, output_dir, 
    ...
    comprehensive_results = {
        ...
        'summary': {
            ...
            'worst_case_scenario': stress_results.loc[stress_results['final_equity'].idxmin(), 'scenario']
        }
    }
