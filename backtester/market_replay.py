...
class MarketReplay:
    ...
    def __init__(
        ...
        self.replay_start_time: datetime | None = None
    ...
    async def load_data(self) -> None:
        ...
        try:
            ...
            for symbol in self.config.symbols:
                ...
                self.logger.info(
                    "Loaded symbol data", symbol=symbol, points=len(symbol_data)
                )
            ...
            self.logger.info(
                ...
                end_time=self.data_queue[-1].timestamp.isoformat()
                if self.data_queue
                else None,
            )
    ...
    async def _load_symbol_data(self, symbol: str) -> list[DataPoint]:
        ...
        if not file_found:
            self.logger.warning(
                "No data file found for symbol", symbol=symbol, patterns=file_patterns
            )
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    self.logger.warning(
                        "Error parsing row from parquet",
                        error=str(e),
                        row=row.to_dict(),
                    )
                    continue
            ...
            self.logger.error(
                "Error reading parquet file", file=str(file_path), error=str(e)
            )
            raise
        return data_points
    ...
    async def _read_data_file(self, file_path: Path, symbol: str) -> list[DataPoint]:
        ...
        try:
            ...
            self.logger.error(
                "Error reading data file", file=str(file_path), error=str(e)
            )
            raise

        return data_points

    async def _parse_data_row(
        self, row: dict[str, str], symbol: str
    ) -> DataPoint | None:
        ...
        try:
            ...
            if self.config.timeframe == TimeFrame.TICK:
                ...
                return DataPoint(
                    timestamp=timestamp, symbol=symbol, data=ohlcv_data, data_type="bar"
                )

        except (ValueError, TypeError, KeyError) as e:
            self.logger.warning(
                "Could not parse data row", error=str(e), row=row, symbol=symbol
            )
            return None

    def _parse_timestamp(self, timestamp_str: Any) -> datetime | None:
    ...
    async def _validate_data(
        self, data_points: list[DataPoint], symbol: str
    ) -> list[DataPoint]:
        ...
        for point in data_points:
            ...
            if point.data_type == "tick":
                ...
                if (
                    tick_data.bid <= 0
                    or tick_data.ask <= 0
                    or tick_data.ask < tick_data.bid
                ):
                    self.validation_errors += 1
                    continue
                ...
                if (
                    ...
                    or bar_data.low <= 0
                    or bar_data.close <= 0
                    or bar_data.high < max(bar_data.open, bar_data.close)
                    or bar_data.low > min(bar_data.open, bar_data.close)
                ):
                    self.validation_errors += 1
                    continue
            ...
            if (
                prev_timestamp
                and self.config.fill_gaps
                and point.timestamp - prev_timestamp
                > timedelta(minutes=self.config.max_gap_minutes)
            ):
                # Fill gap with interpolated data
                gap_points = await self._fill_gap(
                    prev_timestamp, point.timestamp, symbol, point.data_type
                )
    ...
    async def _fill_gap(
        self, start_time: datetime, end_time: datetime, symbol: str, data_type: str
    ) -> list[DataPoint]:
    ...
    def get_progress(self) -> dict[str, int | float | str]:
        ...
        progress_pct = (
            (self.points_streamed / self.total_points * 100)
            if self.total_points > 0
            else 0
        )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        runtime = (
            (datetime.now() - self.replay_start_time).total_seconds()
        ...
        return {
            ...
            "performance": {
                ...
                "points_per_second": self.points_streamed / runtime
                if runtime > 0
                else 0,
            },
            "data_quality": {
                "validation_errors": self.validation_errors,
                "gaps_filled": self.gaps_filled,
                "error_rate": self.validation_errors / self.total_points
                if self.total_points > 0
                else 0,
            },
        }
...
async def replay_data_from_files(
    file_paths: list[str], symbols: list[str], timeframe: str = "1m"
