...
class TestExecutionSimulator:
    ...
    def test_execution_simulator_initialization(
        self, execution_simulator, execution_config
    ):
    ...
    async def test_slippage_calculation(
        self, execution_simulator, sample_order, sample_market_data
    ):
    ...
    async def test_commission_calculation(self, execution_simulator, sample_order):
        ...
        quantity = 100000
        price = 1.1001
        commission = await execution_simulator._calculate_commission(
            sample_order, quantity, price
        )
...
class TestPerformanceMetrics:
    ...
    def test_performance_config_creation(self):
        ...
        config = create_performance_config(
            initial_capital=50000.0, risk_free_rate=0.03, trading_days_per_year=250
        )
    ...
    def test_performance_analyzer_initialization(
        self, performance_analyzer, performance_config
    ):
        ...
        for fill in sample_fills:
            ...
            if fill.symbol not in positions:
                positions[fill.symbol] = Position(
                    symbol=fill.symbol, quantity=0.0, avg_price=0.0
                )
    ...
    def test_rolling_metrics(self, performance_analyzer):
        ...
        for i in range(60):  # 60 days of data
            timestamp = base_time + timedelta(days=i)
            daily_return = (
                0.001 + 0.01 * np.random.normal()
            )  # 0.1% average with 1% volatility
    ...
    def test_strategy_comparison(self):
        ...
        assert (
            comparison.loc["strategy_c", "total_return"]
            > comparison.loc["strategy_a", "total_return"]
        )
...
class TestBacktestEngine:
    ...
    async def test_backtest_engine_initialization_async(self, backtest_engine):
        ...
        assert (
            backtest_engine.state.current_equity
            == backtest_engine.config.initial_capital
        )
    ...
    async def test_equity_calculation(self, backtest_engine):
        ...
        # Add position
        position = Position(symbol="EURUSD", quantity=100000, avg_price=1.1000)
        ...
        expected_equity = (
            100000 - 100000 * 1.1000 - 7.0 + (1.1011 - 1.1000) * 100000
        )  # Using mid price
    ...
    async def test_risk_limit_checking(self, backtest_engine):
        ...
        large_position_size = (
            backtest_engine.config.initial_capital * 2
        )  # 200% of capital

        # Should fail risk check
        risk_check = await backtest_engine._check_risk_limits(
            signal, large_position_size
        )
        assert not risk_check

        # Normal position size should pass
        normal_position_size = (
            backtest_engine.config.initial_capital * 0.05
        )  # 5% of capital
        risk_check = await backtest_engine._check_risk_limits(
            signal, normal_position_size
        )
        assert risk_check
