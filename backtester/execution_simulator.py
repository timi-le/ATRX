...
class ExecutionConfig:
    ...
    weekend_trading: bool = (
        True  # Enable for backtesting (FX is global 24/7 except for brief weekend gaps)
    )
...
class ExecutionSimulator:
    ...
    def __init__(
        ...
        # Market data
        self.current_market_data: dict[str, MarketData | OHLCV] = {}
    ...
    async def submit_order(self, order: Order) -> str:
        ...
        if not await self._validate_order(order):
            ...
            self.logger.warning(
                "Order rejected", order_id=order.order_id, reason="validation_failed"
            )
        ...
        execution_task = asyncio.create_task(
            self._execute_order_after_delay(order, latency)
        )
    ...
    async def _validate_order(self, order: Order) -> bool:
        ...
        if order.quantity > self.config.max_order_size:
            self.logger.debug(
                "Order rejected: quantity too large",
                order_id=order.order_id,
                quantity=order.quantity,
        ...
        if np.random.random() < self.config.rejection_rate:
            self.logger.debug(
                "Order rejected: random rejection",
                order_id=order.order_id,
        ...
        self.logger.debug(
            "Order validation passed", order_id=order.order_id, symbol=order.symbol
        )
        return True
    ...
    async def _execute_order(self, order: Order) -> None:
        ...
        try:
            ...
            slippage = await self._calculate_slippage(
                order, execution_price, market_data
            )
            ...
            commission = await self._calculate_commission(
                order, fill_quantity, final_price
            )
            ...
            self.logger.debug(
                "Fill appended in ExecutionSimulator",
                fill=fill,
                total_fills_now=len(self.fills),
                processed_fill_count_before_this_fill=self.processed_fill_count,
            )
    ...
    async def _calculate_execution_price(
        self, order: Order, market_data: MarketData | OHLCV
    ) -> float:
        ...
        if isinstance(market_data, MarketData):
            ...
            if order.order_type == OrderType.MARKET:
                return (
                    market_data.ask if order.side == OrderSide.BUY else market_data.bid
                )
    ...
    async def _calculate_fill_quantity(self, order: Order) -> float:
        ...
        if np.random.random() < self.config.partial_fill_probability:
            # Partial fill
            fill_percentage = np.random.uniform(self.config.min_fill_percentage, 1.0)
    ...
    async def _calculate_slippage(
        ...
        if order.quantity > self.config.liquidity_impact_threshold:
            impact_factor = (
                order.quantity / self.config.liquidity_impact_threshold
            ) ** 0.5
            market_impact = (
                base_slippage * self.config.market_impact_factor * impact_factor
            )
        else:
            market_impact = 0
        ...
        volatility_impact = (
            base_slippage * volatility * self.config.volatility_slippage_factor
        )
    ...
    async def _update_position(self, fill: Fill) -> None:
        ...
        if new_quantity != 0:
            if (position.quantity >= 0 and fill.side == OrderSide.BUY) or (
                ...
                # Adding to position
                total_cost = (
                    position.quantity * position.avg_price + fill.quantity * fill.price
                )
    ...
    def _get_current_volatility(self, symbol: str) -> float:
        ...
        if (
            symbol not in self.volatility_window
            or len(self.volatility_window[symbol]) < 2
        ):
    ...
    async def wait_for_pending_orders(self, timeout: float = 10.0) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"All {len(self.execution_tasks)} pending orders processed"
            )
        except asyncio.TimeoutError:
            self.logger.warning(
                f"Timeout waiting for {len(self.execution_tasks)} pending orders"
            )
        finally:
    ...
    def get_fills(self) -> list[Fill]:
        ...
        self.logger.debug(
            "ExecutionSimulator.get_fills called",
            total_fills_in_list=len(self.fills),
            processed_fill_count=self.processed_fill_count,
            num_new_fills_returned=len(new_fills),
        )
        return new_fills
    ...
    def mark_fills_processed(self, count: int) -> None:
        ...
        self.processed_fill_count = min(
            self.processed_fill_count + count, len(self.fills)
        )
        self.logger.debug(
            "ExecutionSimulator.mark_fills_processed called",
            count_to_mark=count,
            old_processed_count=old_processed_count,
            new_processed_count=self.processed_fill_count,
            total_fills_in_list=len(self.fills),
        )
    ...
    def get_statistics(self) -> dict[str, int | float]:
        ...
        fill_rate = (
            self.filled_orders / self.total_orders if self.total_orders > 0 else 0
        )
...
class RealisticExecutionSimulator(ExecutionSimulator):
    ...
    async def _calculate_slippage(
        ...
        base_slippage = await super()._calculate_slippage(
            order, execution_price, market_data
        )
