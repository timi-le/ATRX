...
from core.regime_detector import RuleBasedRegimeDetector, RegimeOutput, RegimeFeatures, RegimeType
...
from backtester.market_replay import MarketReplay, ReplayConfig, DataPoint, create_replay_config
from backtester.execution_simulator import ExecutionSimulator, ExecutionConfig, Fill
...
class BacktestConfig:
    ...
    strategies: List[str] = field(default_factory=lambda: ["grid_martingale", "breakout_trend"])
...
class BacktestState:
    ...
    last_market_data: Dict[str, Union[MarketData, OHLCV]]
    data_history: Dict[str, List[DataPoint]] = field(default_factory=dict) # History for feature calculation
    current_regime: Optional[RegimeType] = None
    current_features: Optional[RegimeFeatures] = None
    current_regime_output: Optional[RegimeOutput] = None
    current_ml_prediction: Optional[Prediction] = None
...
class BacktestEngine:
    ...
    def __init__(
        ...
        feature_engine: Optional[HighPerformanceFeatureEngine] = None,
        ...
        risk_manager: Optional[CoreRiskManager] = None,
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.performance_analyzer = PerformanceAnalyzer(performance_config, self.logger)
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Starting backtest run: {self.config.start_date} to {self.config.end_date}, symbols={self.config.symbols}, mode={self.config.mode}")
        
        if not self.market_replay or not self.execution_simulator or not self.performance_analyzer:
        ...
        try:
            ...
            async for data_point in self.market_replay.stream():
                ...
                if data_point_count % 1000 == 0:
                    self.logger.info(f"Processing data point {data_point_count}", symbol=data_point.symbol, timestamp=data_point.timestamp)
                
                await self._process_data_point(data_point)
            
            self.logger.info(f"Backtest loop completed. Total data points processed: {data_point_count}")
    ...
    async def _process_data_point(self, data_point: DataPoint) -> None:
        ...
        self.logger.debug("BacktestEngine._process_data_point entered", 
    ...
    def _update_data_history(self, data_point: DataPoint):
        ...
        if len(self.state.data_history[symbol]) > max_history:
            self.state.data_history[symbol] = self.state.data_history[symbol][-max_history:]

    async def _process_full_pipeline(self, data_point: DataPoint) -> None:
        """Process data through the complete trading pipeline."""
        self.logger.debug("BacktestEngine._process_full_pipeline entered", data_point_symbol=data_point.symbol, data_point_timestamp=data_point.timestamp)
    ...
    async def _compute_features(self, data_point: DataPoint) -> Optional[RegimeFeatures]:
        ...
        try:
            ...
            recent_data_points = self._get_recent_market_data(data_point.symbol, lookback=200) # Need enough data for indicators
            ...
            if features:
                self.logger.debug("Features computed", symbol=data_point.symbol, features=features.to_dict())
    ...
    async def _detect_regime(self, data_point: DataPoint) -> Optional[RegimeOutput]:
        """Detect market regime using the feature set."""
        if not self.regime_detector or not self.state.current_features:
            return None
        
        try:
            regime_output = await self.regime_detector.predict(self.state.current_features)
            self.state.current_regime_output = regime_output
            self.state.current_regime = regime_output.regime
            self.logger.debug("Regime detected", regime=self.state.current_regime.value, confidence=regime_output.confidence)
            return regime_output
        except Exception as e:
            self.logger.error("Error during regime detection", error=str(e), features=self.state.current_features.to_dict())
            return None
    
    async def _predict_signal(self, data_point: DataPoint) -> Optional[Prediction]:
    ...
    async def _process_signal(self, signal: Signal) -> None:
        ...
        if position_size <= 0:
            self.logger.info("Position size is zero or less, skipping order.", signal=signal)
            return
            
        # 2. Check pre-trade risk limits
        risk_ok = await self._check_risk_limits(signal, position_size)
        if not risk_ok:
            self.logger.warning("Trade rejected due to pre-trade risk checks.", signal=signal, size=position_size)
            return
    ...
    async def _calculate_position_size(self, signal: Signal) -> float:
        ...
        trade_signal_input = TradeSignalInput(
            ...
            reward_risk_ratio=signal.take_profit_pips / signal.stop_loss_pips if signal.stop_loss_pips > 0 else None,
            current_price=signal.price,
            volatility_atr=self.state.current_features.atr if self.state.current_features else 0,
    ...
    async def _check_risk_limits(self, signal: Signal, position_size: float) -> bool:
        ...
        if not self.risk_manager:
            # Basic risk checks
            total_exposure = sum(abs(pos.quantity * pos.avg_price) for pos in self.state.current_positions.values())
    ...
    async def _create_order(self, signal: Signal, position_size: float) -> Optional[Order]:
        ...
        try:
            ...
            if fill_count > 0:
                ...
                if self.execution_simulator.processed_fill_count % 1000 == 0:
                    self.logger.info(f"Processed {self.execution_simulator.processed_fill_count} fills, current equity: ${self.performance_analyzer.current_equity:.2f}")
    ...
    def _get_recent_market_data(self, symbol: str, lookback: int = 200) -> Optional[List[DataPoint]]:
    ...
    async def _track_theoretical_performance(self, signal: Signal, data_point: DataPoint) -> None:
    ...
    async def _finalize_backtest(self) -> None:
        ...
        try:
            ...
            regime_performance = self.performance_analyzer.calculate_regime_performance()
