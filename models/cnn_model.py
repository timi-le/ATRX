...
class CNNPredictor(BasePredictorModel):
    ...
    def __init__(
        ...
        self.cnn_filters = getattr(config, 'cnn_filters', 64)  # Increased for better pattern detection
        self.kernel_sizes = getattr(config, 'kernel_sizes', [2, 3, 5, 8])  # FX-specific timeframes
        self.pool_size = getattr(config, 'pool_size', 2)
        self.num_conv_layers = getattr(config, 'num_conv_layers', 2)  # Reduced for speed
        self.dropout_rate = getattr(config, 'dropout_rate', 0.3)
        self.use_batch_norm = getattr(config, 'use_batch_norm', True)
        self.use_residual = getattr(config, 'use_residual', True)  # Residual connections
    ...
    def build_model(self) -> "keras.Model":
            ...
            for j in range(1, self.num_conv_layers):
                ...
                if j % 2 == 0:
                    x = layers.MaxPooling1D(
                        ...
                        name=f"maxpool_k{kernel_size}_{j+1}"
                    )(x)
        ...
        if self.use_attention:
            ...
            attention_input = layers.Reshape((len(self.kernel_sizes), self.cnn_filters))(merged)
            attention = layers.MultiHeadAttention(
                num_heads=4,
                key_dim=self.cnn_filters // 4,
                name="attention"
...
def create_cnn_config(
    ...
    **kwargs
    ...
    standard_params = {
        ...
        'early_stopping_patience': kwargs.get('early_stopping_patience', 7),  # More patience for CNN
