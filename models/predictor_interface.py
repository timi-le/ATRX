...
from core.interfaces.ml_interfaces import Features, MLPredictor, Prediction
...
class BasePredictorModel(MLPredictor):
    ...
    async def predict(
        ...
        if isinstance(features, Features):
            ...
            if X.ndim == 1:
                ...
                ) and self.config.model_type.value in ["lstm", "cnn"]:
                    # For sequence models, repeat the features to create a sequence
                    X = np.tile(X, (self.config.sequence_length, 1)).reshape(
    ...
    async def predict_batch(
        ...
        if isinstance(features_batch, list) and isinstance(features_batch[0], Features):
            ...
            confidences = [
                self._calculate_confidence(X[i : i + 1], pred_values[i])
    ...
    def prepare_sequences(
        self, data: np.ndarray, sequence_length: int | None = None
    ) -> np.ndarray:
    ...
    def evaluate_model(
        ...
        if len(y_test) > 1:
            returns = (
                y_pred
                if self.config.prediction_type == PredictionType.RETURN_REGRESSION
                else y_test
            )
