...
from core.interfaces.trading_interfaces import Order, Position
from core.interfaces.trading_interfaces import RiskManager as RiskManagerInterface
...
class CoreRiskManager(RiskManagerInterface):
    ...
    async def check_pre_trade_risk(
        ...
        try:
            ...
            current_symbol_exposure = self._calculate_symbol_exposure(
                order.symbol, current_positions
            )
    ...
    async def monitor_drawdown(self, current_pnl: float, peak_pnl: float) -> bool:
        ...
        try:
            ...
            daily_drawdown = (
                self.daily_start_capital - (self.initial_capital + current_pnl)
            ...
            if daily_drawdown >= self.config.max_daily_drawdown:
                ...
                await self._send_alert(
                    ...
                    "daily_drawdown",
                    daily_drawdown,
                    self.config.warning_daily_drawdown,
                )
            ...
            if total_drawdown >= self.config.max_total_drawdown:
                ...
                await self._send_alert(
                    ...
                    "total_drawdown",
                    total_drawdown,
                    self.config.warning_total_drawdown,
                )
    ...
    async def calculate_var(
        ...
        try:
            ...
            pnl_changes = []
            for i in range(
                1, min(len(self.pnl_history), self.config.var_lookback_period)
            ):
            ...
            if self.config.var_calculation_method == "historical":
                ...
                var_scaled = abs(mean_return + z_score * std_return) * np.sqrt(
                    horizon_days
                )
            ...
            if portfolio_var >= self.config.max_portfolio_var:
                ...
                await self._send_alert(
                    ...
                    "portfolio_var",
                    portfolio_var,
                    self.config.warning_portfolio_var,
                )
        ...
        try:
            ...
            for symbol, position in positions.items():
                exposure = PositionExposure(
                    ...
                    percentage_of_capital=abs(position.quantity * position.avg_price)
                ...
                if len(symbol) == 6:  # FX pair format like EURUSD
                    ...
                    if position.quantity > 0:  # Long position
                        self.currency_exposures[
                            base_currency
                        ] += exposure.percentage_of_capital
                        self.currency_exposures[
                            quote_currency
                        ] -= exposure.percentage_of_capital
                    else:  # Short position
    ...
    async def update_pnl(
        self, total_pnl: float, realized_pnl: float, unrealized_pnl: float
    ) -> None:
        """Update P&L tracking."""
        try:
            # Store P&L history
            ...
            await self.monitor_drawdown(
                total_pnl, self.peak_capital - self.initial_capital
            )
    ...
    async def get_risk_metrics(self) -> RiskMetrics:
        ...
        try:
            ...
            total_pnl = self.total_realized_pnl + sum(
            ...
            current_drawdown = max(
                0,
                (self.peak_capital - (self.initial_capital + total_pnl))
                / self.peak_capital,
            )
            max_drawdown = (
                max([dd["total_drawdown"] for dd in self.drawdown_history])
            ...
            leverage = self._calculate_leverage(
                self.current_positions, self.current_capital
            )
            ...
            portfolio_var = (
                self.var_history[-1]["var_percentage"] if self.var_history else 0
            )

            # Determine risk status
            risk_status = self._determine_risk_status(
                current_drawdown, total_exposure, leverage, portfolio_var
            )
    ...
    async def set_manual_override(
        self, duration_hours: int = 24, reason: str = "Manual override"
    ) -> None:
        """Set manual override for risk controls."""
        try:
            self.manual_override_active = True
            self.manual_override_until = datetime.now() + timedelta(
    ...
    async def clear_manual_override(self) -> None:
        ...
        try:
            ...
            await self._send_alert(
                RiskLevel.INFO, "Manual override cleared", "manual_override", 0.0, 1.0
            )
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "manual_override_until": self.manual_override_until.isoformat()
    ...
    def _calculate_total_exposure(self, positions: dict[str, Position]) -> float:
        ...
        total_value = sum(
            abs(pos.quantity * pos.avg_price) for pos in positions.values()
        )
        return total_value / self.current_capital

    def _calculate_leverage(
        self, positions: dict[str, Position], capital: float
    ) -> float:
    ...
    def _determine_risk_status(
        self, drawdown: float, exposure: float, leverage: float, var: float
    ) -> RiskStatus:
    ...
    async def _check_rapid_loss(self) -> None:
        ...
        try:
            ...
            rapid_loss_cutoff = current_time - timedelta(
                seconds=self.config.rapid_loss_timeframe
            )
            ...
            if loss_percentage >= self.config.rapid_loss_threshold:
                await self._send_alert(
                    ...
                    "rapid_loss",
                    loss_percentage,
                    self.config.rapid_loss_threshold,
                )
    ...
    async def _trigger_emergency_stop(self, reason: str) -> None:
        ...
        try:
            ...
            self.logger.critical(
                "Emergency stop triggered", reason=reason, timestamp=datetime.now()
            )
    ...
    async def _check_recovery_conditions(self) -> bool:
        ...
        try:
            ...
            stability_cutoff = datetime.now() - timedelta(
                minutes=self.config.recovery_conditions["stability_period_minutes"]
            )

            recent_alerts = [
                alert
                for alert in self.alert_history
                if alert.timestamp >= stability_cutoff
                and alert.level in [RiskLevel.CRITICAL, RiskLevel.EMERGENCY]
            ]
    ...
    async def _send_alert(
        ...
        try:
            ...
            if alert_key in self.last_alert_times:
                ...
                cooldown = self.config.alert_frequency_limits[
                    "duplicate_alert_cooldown"
                ]

                if level == RiskLevel.EMERGENCY:
                    cooldown = self.config.alert_frequency_limits[
                        "emergency_alert_cooldown"
                    ]
