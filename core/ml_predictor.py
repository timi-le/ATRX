...
class XGBoostPredictor(MLPredictor):
    ...
    def __init__(
        ...
        if model_path:
            ...
            self.logger.info("Initialized XGBoostPredictor for training", params=default_params)
...
class LSTMPredictor(MLPredictor):
    ...
    def __init__(
        ...
        if model_path:
            ...
            self.logger.info("Initialized LSTMPredictor for training", timesteps=timesteps, n_features=n_features)

    def _build_model(self):
        model = Sequential([
            LSTM(50, input_shape=(self.timesteps, self.n_features), return_sequences=True),
            Dropout(0.2),
            LSTM(50),
            Dropout(0.2),
    ...
    def train(self, X: pd.DataFrame, y: pd.Series):
        ...
        if X_seq.shape[0] == 0:
            self.logger.warning("Not enough data to create sequences for training.", data_points=len(X))
            return
    ...
    def predict(self, X: pd.DataFrame) -> Prediction:
        ...
        if len(X) < self.timesteps:
            self.logger.warning("Not enough data for prediction.", required=self.timesteps, available=len(X))
            return Prediction(signal=0, probability=0.0)

        # Use the most recent `timesteps`
        X_pred = X.tail(self.timesteps).values.reshape(1, self.timesteps, self.n_features)
...
class CNNPredictor(MLPredictor):
    ...
    def __init__(
        ...
        if model_path:
            ...
            self.n_features = n_features
            self.model = self._build_model()
            self.logger.info("Initialized CNNPredictor for training", image_size=image_size, n_features=n_features)

    def _build_model(self):
        model = Sequential([
            Conv2D(32, (2, 2), activation='relu', input_shape=(self.image_size, self.image_size, 1)),
            MaxPooling2D((2, 2)),
            Flatten(),
    ...
    def _create_dataset(self, X: pd.DataFrame, y: pd.Series):
        ...
        if X.shape[1] < self.n_features:
            padding = pd.DataFrame(np.zeros((num_samples, self.n_features - X.shape[1])), index=X.index)
            X_padded = pd.concat([X, padding], axis=1)
        else:
            X_padded = X.iloc[:, :self.n_features]

        X_images = X_padded.values.reshape(num_samples, self.image_size, self.image_size, 1)
    ...
    def predict(self, X: pd.DataFrame) -> Prediction:
        ...
        if X.shape[1] < self.n_features:
            padding = pd.DataFrame(np.zeros((len(X), self.n_features - X.shape[1])), index=X.index)
            X_padded = pd.concat([X, padding], axis=1)
        else:
            X_padded = X.iloc[:, :self.n_features]
