...
class TradeSignalInput:
    ...
    take_profit_pips: Optional[float]      # Take profit in pips
    stop_loss_pips: Optional[float]        # Stop loss in pips
    reward_risk_ratio: Optional[float]     # TP/SL ratio (if pips not available)
    win_probability: Optional[float]       # Historical or ML-estimated win rate
    current_price: float
...
class PositionSizerConfig:
    ...
    def __init__(self, config_path: str = "config/risk_settings.yaml"):
        ...
        self.emergency_mode_duration_hours = emergency_config['emergency_mode_duration_hours']
...
class KellyPositionSizer:
    ...
    def __init__(
        ...
        self.emergency_mode_until: Optional[datetime] = None
        self.last_drawdown_check: Optional[datetime] = None
    ...
    async def calculate_position_size(
        ...
        try:
            ...
            kelly_raw = self._calculate_kelly_fraction(win_probability, reward_risk_ratio)
            
            # Apply Kelly scaling
            kelly_scaled = self._apply_kelly_scaling(kelly_raw)
            
            # Apply volatility scaling
            volatility_regime, volatility_scaling = self._calculate_volatility_scaling(signal)
            
            # Apply drawdown protection
            drawdown_level, drawdown_scaling = self._calculate_drawdown_scaling(portfolio_state)
            ...
            confidence = self._calculate_sizing_confidence(
                signal, win_probability, reward_risk_ratio, volatility_regime, drawdown_level
            )
    ...
    def _calculate_kelly_fraction(self, win_probability: float, reward_risk_ratio: float) -> float:
    ...
    def _apply_kelly_scaling(self, kelly_raw: float) -> float:
        ...
        scaling_factor = profile_config.get('kelly_scaling_factor', self.config.kelly_scaling_factor)
        
        return kelly_raw * scaling_factor
    
    def _calculate_volatility_scaling(self, signal: TradeSignalInput) -> Tuple[VolatilityRegime, float]:
        """Calculate volatility-based position scaling."""
        
        if not self.config.volatility_scaling_enabled or len(self.volatility_history) < 5:
        ...
        high_percentile = np.percentile(volatility_array, self.config.high_vol_percentile)
    ...
    def _calculate_drawdown_scaling(self, portfolio_state: PortfolioState) -> Tuple[DrawdownLevel, float]:
    ...
    def _apply_constraints(
        ...
        if position_size < self.config.min_position_size:
            ...
            constraints_applied.append(f"min_position_size_{self.config.min_position_size}")
        ...
        current_exposure = sum(pos.get('size', 0) for pos in portfolio_state.open_positions)
    ...
    def _calculate_sizing_confidence(
        ...
        confidence = base_confidence * parameter_quality * volatility_adjustment * drawdown_adjustment
    ...
    def _is_emergency_mode(self, portfolio_state: PortfolioState) -> bool:
        ...
        if len(self.volatility_history) >= 5:
            ...
            historical_volatility = np.mean(list(self.volatility_history)[:-5]) if len(self.volatility_history) > 5 else recent_volatility
            
            if historical_volatility > 0 and recent_volatility / historical_volatility >= self.config.volatility_spike_threshold:
    ...
    def _activate_emergency_mode(self, reason: str) -> None:
        ...
        self.emergency_mode_until = datetime.now() + timedelta(hours=self.config.emergency_mode_duration_hours)
        self.logger.warning(f"Emergency mode activated: {reason}", until=self.emergency_mode_until)
    
    def _create_emergency_result(self, signal: TradeSignalInput, reason: str) -> PositionSizeResult:
    ...
    def _update_performance_tracking(self, result: PositionSizeResult) -> None:
        ...
        if result.kelly_raw > 0:
            self.average_kelly_fraction = (
                (self.average_kelly_fraction * (self.total_sizing_decisions - 1) + result.kelly_raw) /
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        recent_results = self.sizing_history[-100:] if len(self.sizing_history) >= 100 else self.sizing_history
        ...
        return {
            ...
            "average_position_size": np.mean([r.position_size for r in recent_results]) if recent_results else 0.0,
            "average_confidence": np.mean([r.confidence for r in recent_results]) if recent_results else 0.0,
            "emergency_mode_active": self.emergency_mode_until is not None and datetime.now() < self.emergency_mode_until,
    ...
    def update_risk_profile(self, new_profile: RiskProfile) -> None:
        ...
        self.logger.info(f"Risk profile updated from {old_profile.value} to {new_profile.value}")
