...
def generate_triple_barrier_labels(
    ...
    if data['atr'].isnull().any():
        _logger.warning("ATR column contains NaNs. Filling with a forward fill strategy.")
    ...
    label_dist = {str(k): v for k, v in labels.value_counts(normalize=True).to_dict().items()}
