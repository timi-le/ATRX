...
class MT5Interface(BrokerInterface):
    ...
    def __init__(self, config: BrokerConfig, logger: Optional[structlog.stdlib.BoundLogger] = None):
        self.config = config
    ...
    async def submit_order(self, order: Order) -> BrokerOrder:
        ...
        try:
            ...
            mt5_request = self._convert_to_mt5_order(order)
            ...
            self.logger.error("Error submitting order to MT5", order_id=order.order_id, error=str(e))
            raise
    ...
    async def cancel_order(self, broker_order_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error("Error cancelling order in MT5", broker_order_id=broker_order_id, error=str(e))
            return False
    ...
    async def get_order_status(self, broker_order_id: str) -> OrderStatus:
        ...
        try:
            ...
            self.logger.error("Error getting order status from MT5", broker_order_id=broker_order_id, error=str(e))
            return OrderStatus.REJECTED
    ...
    async def get_market_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("Error getting market data from MT5", symbol=symbol, error=str(e))
            return {}
...
class IBKRInterface(BrokerInterface):
    ...
    def __init__(self, config: BrokerConfig, logger: Optional[structlog.stdlib.BoundLogger] = None):
        ...
        try:
            from services.execution.fix_connector.fix_order_router import create_fix_order_router
        ...
        self.order_id_mapping: Dict[str, str] = {}  # broker_order_id -> internal_order_id
    ...
    async def submit_order(self, order: Order) -> BrokerOrder:
        ...
        try:
            ...
            self.logger.error("Error submitting order to IBKR", order_id=order.order_id, error=str(e))
            raise
    ...
    async def cancel_order(self, broker_order_id: str) -> bool:
        ...
        try:
            ...
            if not internal_order_id:
                self.logger.warning("Order not found for cancellation", broker_order_id=broker_order_id)
                return False
            ...
            if result["success"]:
                self.logger.info("Order cancelled in IBKR", broker_order_id=broker_order_id)
                return True
            else:
                self.logger.error("Order cancellation failed", broker_order_id=broker_order_id, error=result.get("error"))
                return False
                
        except Exception as e:
            self.logger.error("Error cancelling order in IBKR", broker_order_id=broker_order_id, error=str(e))
            return False
    ...
    async def get_order_status(self, broker_order_id: str) -> OrderStatus:
        ...
        try:
            ...
            self.logger.error("Error getting order status from IBKR", broker_order_id=broker_order_id, error=str(e))
            return OrderStatus.REJECTED
    ...
    async def get_account_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'total_executions_received': account_info.get('total_executions_received', 0),
    ...
    async def get_market_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("Error getting market data from IBKR", symbol=symbol, error=str(e))
            return {}
    
    # FIX callback handlers
    def _on_order_update(self, order_id: str, status: 'OrderStatus', details: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error("Error handling order update", order_id=order_id, error=str(e))
    
    def _on_execution(self, order_id: str, fill_qty: float, fill_price: float, timestamp: datetime):
        ...
        try:
            ...
            self.logger.error("Error handling execution", order_id=order_id, error=str(e))
    ...
    def _on_error(self, order_id: str, error_message: str):
        ...
        try:
            ...
            self.logger.error("Order error received", order_id=order_id, error=error_message)
            
        except Exception as e:
            self.logger.error("Error handling order error", order_id=order_id, error=str(e))
...
class OrderRouter:
    ...
    def __init__(
        ...
        self.logger.info(
            ...
            enabled_brokers=[bt for bt, bc in self.broker_configs.items() if bc.enabled]
        )
        ...
        for broker_name, broker_config in broker_configs.items():
            try:
                ...
                config = BrokerConfig(
                    ...
                    max_orders_per_second=broker_config.get('max_orders_per_second', 10.0),
                ...
                self.logger.error(f"Failed to initialize broker {broker_name}", error=str(e))
        ...
        try:
            ...
            for broker_type, broker in self.brokers.items():
                ...
                if config.enabled:
                    ...
                    if success:
                        self.broker_health[broker_type]['status'] = ConnectionStatus.CONNECTED
                        self.logger.info(f"Connected to {broker_type.value}")
                    else:
                        self.broker_health[broker_type]['status'] = ConnectionStatus.ERROR
    ...
    async def stop(self) -> None:
        ...
        try:
            for broker_type, broker in self.brokers.items():
                ...
                self.broker_health[broker_type]['status'] = ConnectionStatus.DISCONNECTED
    ...
    async def route_order(self, order: Order) -> BrokerOrder:
        ...
        try:
            ...
            self.routing_stats['avg_routing_time'] = (
                (self.routing_stats['avg_routing_time'] * (self.routing_stats['orders_routed'] - 1) + routing_time) /
            ...
            self.logger.error("Failed to route order", order_id=order.order_id, error=str(e))
            raise
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info("Order cancelled", order_id=order_id, broker=broker_type.value)
    ...
    async def get_order_status(self, order_id: str) -> OrderStatus:
        ...
        try:
            ...
            self.logger.error("Error getting order status", order_id=order_id, error=str(e))
            return OrderStatus.REJECTED
        ...
        for broker_type, broker in self.brokers.items():
            if self.broker_health[broker_type]['status'] == ConnectionStatus.CONNECTED:
                try:
                    ...
                    self.logger.error(f"Error getting positions from {broker_type.value}", error=str(e))
    ...
    async def get_account_info(self, broker_type: BrokerType) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error getting account info from {broker_type.value}", error=str(e))
            return {}
    
    async def _select_broker(self, order: Order) -> Optional[BrokerType]:
            ...
            try:
                for broker_type, broker in self.brokers.items():
                    ...
                    if is_connected:
                        ...
                        config = self.broker_configs[broker_type]
                        if config.enabled and health['error_count'] <= config.retry_attempts:
                            self.logger.info(f"Attempting to reconnect to {broker_type.value}")
    ...
    def get_routing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'connected_brokers': [
                bt.value for bt, health in self.broker_health.items()
