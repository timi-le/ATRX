...
def serialize_message_data(obj: Any) -> Any:
    ...
    if isinstance(obj, datetime):
        ...
        return serialize_message_data(
            asdict(obj) if hasattr(obj, "__dataclass_fields__") else obj.__dict__
        )
    else:
        return obj
...
class ZMQPublisher:
    ...
    async def stop(self) -> None:
        ...
        self.context.term()
        self.logger.info(
            "ZMQ Publisher stopped",
            messages_sent=self._message_count,
            uptime=time.time() - self._start_time if self._start_time else 0,
        )

    async def publish_market_data(
        self, data: MarketData, topic: str = Topics.MARKET_DATA_TICKS
    ) -> None:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to publish market data", error=str(e), symbol=data.symbol
            )
            raise

    async def publish_ohlcv_data(
        self, data: OHLCV, topic: str = Topics.MARKET_DATA_BARS
    ) -> None:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to publish OHLCV data", error=str(e), symbol=data.symbol
            )
            raise
            ...
            try:
                ...
                if message_handler:
                    try:
                        ...
                        self.logger.error(
                            "Message handler failed", error=str(e), topic=topic
                        )
