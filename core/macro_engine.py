...
from data.news_sentiment_parser import NewsSentimentParser, MockNewsProvider, SimpleSentimentAnalyzer
...
class HighPerformanceMacroEngine(MacroEngine):
    ...
    def __init__(
        self,
        economic_providers: Optional[List[MacroDataProvider]] = None,
        news_providers: Optional[List[NewsProvider]] = None,
        ...
        self.news_parser = NewsSentimentParser(
            ...
            sentiment_analyzer=sentiment_analyzer or SimpleSentimentAnalyzer(self.logger),
            logger=self.logger
        )
        ...
        # Data storage
        self.economic_events: Dict[Currency, deque] = defaultdict(lambda: deque(maxlen=surprise_window))
        self.news_events: Dict[Currency, deque] = defaultdict(lambda: deque(maxlen=sentiment_window))
        self.surprise_history: Dict[Currency, deque] = defaultdict(lambda: deque(maxlen=surprise_window))
        self.sentiment_history: Dict[Currency, deque] = defaultdict(lambda: deque(maxlen=sentiment_window))
        
        # Cache for latest features
        self._latest_macro_vector: Optional[MacroFeatureVector] = None
    ...
    async def update_economic_event(self, event: EconomicEvent) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error updating economic event: {e}", event_id=event.event_id)
    ...
    async def update_news_event(self, news: NewsEvent) -> None:
        ...
        try:
            ...
            for currency in news.currencies_mentioned:
                ...
                weighted_sentiment = (news.sentiment_score or 0.0) * impact_weight * confidence_weight
    ...
    async def calculate_surprise_score(self, event: EconomicEvent) -> float:
        ...
        try:
            ...
            same_type_surprises = [
                h['raw_surprise'] for h in currency_history
                if h.get('event_type') == event.event_type and h.get('raw_surprise') is not None
            ]
            
            if len(same_type_surprises) < 5:  # Not enough history, use simple normalization
                # Use percentage surprise if available, otherwise simple normalization
                if event.surprise_pct is not None:
                    normalized_surprise = event.surprise_pct / 100.0  # Convert to decimal
                else:
                ...
                if len(same_type_surprises) > 1:
                    ...
                    if std_surprise > 0:
                        normalized_surprise = (event.surprise - mean_surprise) / std_surprise
                    else:
                        normalized_surprise = 0.0
                else:
    ...
    async def get_latest_macro_vector(self, currency: Optional[Currency] = None) -> MacroFeatureVector:
        ...
        if (self._latest_macro_vector and self._last_update_time and 
            (now - self._last_update_time).total_seconds() < 300):
        ...
        for curr in Currency:
            ...
            recent_surprises = [
                h['surprise_score'] for h in list(self.surprise_history[curr])
                if h['surprise_score'] is not None and 
                (now - h['timestamp']).total_seconds() < 86400 * 7  # Last 7 days
            ]
            
            if recent_surprises:
                currency_surprises[curr] = recent_surprises[-1]  # Most recent
                rolling_surprise_means[curr] = statistics.mean(recent_surprises)
                rolling_surprise_stds[curr] = statistics.stdev(recent_surprises) if len(recent_surprises) > 1 else 1.0
            else:
            ...
            recent_sentiments = [
                h['sentiment_score'] for h in list(self.sentiment_history[curr])
                if h['sentiment_score'] is not None and 
                (now - h['timestamp']).total_seconds() < 86400 * 3  # Last 3 days
            ]
            ...
            recent_econ_events = [
                e for e in list(self.economic_events[curr])
                if (now - e.timestamp).total_seconds() < 86400
            ]
            
            recent_news_events = [
                n for n in list(self.news_events[curr])
        ...
        self.logger.debug("Generated latest macro feature vector", timestamp=now.isoformat())
    ...
    async def get_surprise_history(
        ...
        cached_events = [
            e for e in list(self.economic_events[currency])
    ...
    async def get_currency_correlations(self) -> pd.DataFrame:
        ...
        try:
            ...
            for currency in Currency:
                surprise_data = [
                    h['surprise_score'] for h in list(self.surprise_history[currency])
    ...
    async def get_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            for currency in Currency:
                ...
                sentiment_data = [
                    h['sentiment_score'] for h in list(self.sentiment_history[currency])
...
def create_macro_engine(
    ...
    if use_mock_data:
        ...
        from data.econ_calendar_loader import TradingEconomicsProvider, ForexFactoryDataProvider
        from data.news_sentiment_parser import RSSNewsProvider, NewsAPIProvider
