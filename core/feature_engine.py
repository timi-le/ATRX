...
from core.interfaces.ml_interfaces import FeatureEngineer, Features
from core.interfaces.data_interfaces import OHLCV, MarketData
from core.regime_detector import RegimeFeatures
...
def _compute_rolling_kurtosis_skew(prices: np.ndarray, window: int) -> Tuple[np.ndarray, np.ndarray]:
...
def _compute_bollinger_width(upper_band: np.ndarray, lower_band: np.ndarray, middle_band: np.ndarray) -> np.ndarray:
    """Compute normalized Bollinger Bandwidth."""
    return (upper_band - lower_band) / middle_band

def _compute_spread_vs_range(high: np.ndarray, low: np.ndarray, spread: np.ndarray, window: int) -> np.ndarray:
...
@jit(nopython=True, cache=True)
def _compute_bollinger_bands(prices: np.ndarray, period: int, std_dev: float = 2.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
@jit(nopython=True, cache=True)
def _compute_stochastic_oscillator(high: np.ndarray, low: np.ndarray, close: np.ndarray, 
...
class HighPerformanceFeatureEngine:
    ...
    def __init__(self, config: FeatureConfig = None, logger: Optional[structlog.stdlib.BoundLogger] = None):
        """
    ...
    async def compute_technical_features(self, data: pd.DataFrame, window_sizes: List[int] = None) -> pd.DataFrame:
        """Compute all technical features. The interface is async, but core logic is sync for MTF."""
        if window_sizes:
            self.logger.warning("`window_sizes` is ignored; using FeatureConfig.", provided_windows=window_sizes)
        return self._compute_technical_features_sync(data)

    async def compute_volatility_features(self, data: pd.DataFrame, lookback_periods: List[int] = None) -> pd.DataFrame:
        """Compute volatility-based features."""
        if lookback_periods:
             self.logger.warning("`lookback_periods` is ignored; using FeatureConfig.", provided_periods=lookback_periods)
        
        feature_dict = self._compute_volatility_features(data)
        return pd.DataFrame(feature_dict, index=data.index)

    async def compute_momentum_features(self, data: pd.DataFrame, periods: List[int] = None) -> pd.DataFrame:
        """Compute momentum indicators."""
        if periods:
            self.logger.warning("`periods` is ignored; using FeatureConfig.", provided_periods=periods)

        feature_dict = self._compute_momentum_features(data)
        return pd.DataFrame(feature_dict, index=data.index)

    async def compute_carry_features(self, fx_data: pd.DataFrame, interest_rates: pd.DataFrame) -> pd.DataFrame:
        """This engine does not compute carry features."""
        self.logger.warning("compute_carry_features is not implemented in HighPerformanceFeatureEngine.")
        return pd.DataFrame(index=fx_data.index)

    async def compute_macro_surprises(self, economic_data: pd.DataFrame, expectations: pd.DataFrame) -> pd.DataFrame:
        """This engine does not compute macro surprise features."""
        self.logger.warning("compute_macro_surprises is not implemented in HighPerformanceFeatureEngine.")
    ...
    def _compute_volatility_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        for window in self.config.volatility_windows:
            features[f'volatility_{window}'] = _compute_rolling_volatility(returns, window)
        return features
    ...
    def _compute_kurtosis_skew_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        features = {}
        prices_np = data['close'].to_numpy()
    ...
    def _compute_technical_features_sync(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for name, func in computations.items():
            try:
                ...
                if feature_dict:
                    all_feature_sets.append(pd.DataFrame(feature_dict, index=data.index))
            except Exception as e:
                self.logger.error(f"Failed to compute '{name}' features", error=e, exc_info=False)
    ...
    async def compute_mtf_features(
        ...
        if primary_tf not in mtf_data:
            raise ValueError(f"Primary timeframe '{primary_tf}' not found in mtf_data keys.")
        ...
        if 'time' in primary_df.columns:
            ...
            raise ValueError("Primary dataframe must have a datetime index or 'time' column")
        ...
        for tf_str, df in mtf_data.items():
            ...
            if 'time' in tf_df.columns:
                ...
                raise ValueError(f"Timeframe {tf_str} dataframe must have a datetime index or 'time' column")
        ...
        self.logger.info(f"Successfully computed and aligned features from {list(mtf_data.keys())}.")
        return final_df.dropna()
    
    def add_custom_feature(self, name: str, compute_func: Callable) -> None:
        """Adds a custom feature computation function."""
        self.logger.warning("Custom features are not fully supported with the new MTF engine yet.")
