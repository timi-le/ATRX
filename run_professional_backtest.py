...
class ProfessionalBacktester:
    ...
    async def run_comprehensive_backtest(self):
        ...
        for i, config in enumerate(test_configs):
            if results[i]:
                ...
                logger.error(
                    f"Backtest failed for {config['name']} and returned no result."
                )
    ...
    async def _run_single_backtest(self, config: dict) -> dict:
        ...
        backtest_config = BacktestConfig(
            ...
            strategy_weights={
                s: 1.0 / len(config["strategies"]) for s in config["strategies"]
            },
            # Risk management
        ...
        if not market_replay:
            logger.error(
                f"Failed to create market replay for {config['name']}. Skipping."
            )
            return {}
    ...
    async def _create_strategy_switcher(
    ...
    async def _create_mt5_market_replay(self, config: dict) -> Optional["MarketReplay"]:
        ...
        if not all([mt5_login, mt5_password, mt5_server]):
            logger.error(
                "MT5 credentials not found in environment variables. Make sure you have a .env file with MT5_LOGIN, MT5_PASSWORD, and MT5_SERVER."
            )
            return None
        ...
        for symbol in config["symbols"]:
            ...
            if data.empty:
                logger.warning(
                    f"No data downloaded for {symbol}. It might not be available on the server."
                )
            else:
        ...
        if not all_data:
            logger.error(
                "No data downloaded for any symbols. Cannot proceed with backtest."
            )
            return None
        ...
        class MarketReplay:
            def __init__(self, data: dict[str, pd.DataFrame]):
                ...
                self._timeline = (
                    pd.concat(combined_data).sort_values(by="time").reset_index()
                )
            ...
            async def stream(self):
                logger.info(
                    f"Streaming {len(self._timeline)} data points across {len(self._all_symbols)} symbols."
                )
    ...
    async def _generate_comprehensive_report(self):
        ...
        with open(summary_file, "w") as f:
            ...
            best_strategy = max(
                self.results.items(), key=lambda x: x[1]["performance"]["sharpe_ratio"]
            )
            f.write(f"Best Strategy (by Sharpe Ratio): {best_strategy[0]}\n")
            f.write(
                f"  Sharpe Ratio: {best_strategy[1]['performance']['sharpe_ratio']:.2f}\n"
            )
            f.write(
                f"  Total Return: {best_strategy[1]['performance']['total_return']:.2%}\n"
            )
            f.write(
                f"  Max Drawdown: {best_strategy[1]['performance']['max_drawdown']:.2%}\n\n"
            )
            ...
            f.write("-" * 30 + "\n")
            avg_sharpe = np.mean(
                [r["performance"]["sharpe_ratio"] for r in self.results.values()]
            )
            avg_drawdown = np.mean(
                [r["performance"]["max_drawdown"] for r in self.results.values()]
            )
            avg_volatility = np.mean(
                [r["performance"]["volatility"] for r in self.results.values()]
            )
            ...
            avg_fill_rate = np.mean(
                [r["execution"]["fill_rate"] for r in self.results.values()]
            )
            avg_slippage = np.mean(
                [r["execution"]["avg_slippage_per_fill"] for r in self.results.values()]
            )
            total_commission = sum(
                [r["performance"]["total_commission"] for r in self.results.values()]
            )
            ...
            if avg_sharpe > 1.0:
                ...
                f.write(
                    "[WARNING] Consider optimizing strategies for better risk-adjusted returns\n"
                )
            ...
            if avg_drawdown < 0.15:
                ...
                f.write(
                    "[WARNING] High drawdown levels - consider reducing position sizes\n"
                )
