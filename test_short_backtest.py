...
from backtester import BacktestConfig, BacktestEngine, BacktestMode
from backtester.market_replay import DataPoint
from core.interfaces.data_interfaces import OHLCV
from core.interfaces.trading_interfaces import OrderSide, Signal

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.FileHandler("short_backtest.log"), logging.StreamHandler()],
)
...
async def run_short_test():
    ...
    if perf["total_trades"] > 0:
        ...
        logger.info(
            f"✅ Generated {perf['total_trades']} completed trades from {exec_stats['total_fills']} fills"
        )
    ...
    async def generate_signal(self, market_data, features, regime):
        ...
        if self.signal_counter % 5 == 0:  # Signal every 5 data points
            ...
            if self.current_position == 0:
                ...
                logger.info(
                    f"Opening position: {side.value}, net position now: {self.current_position}"
                )
            else:
                ...
                if random.random() < 0.25:  # 25% chance to close position
                    ...
                    if self.current_position > 0:
                        ...
                        logger.info(
                            f"Closing LONG position with SELL signal, trade #{self.trades_completed} completed"
                        )
                    else:
                        side = OrderSide.BUY
                        self.current_position = 0  # Position will be closed
                        self.trades_completed += 1
                        logger.info(
                            f"Closing SHORT position with BUY signal, trade #{self.trades_completed} completed"
                        )
                else:
            ...
            logger.info(
                f"Signal #{self.signals_generated}: {signal.symbol} {signal.side.value} (trades completed: {self.trades_completed})"
            )
            return signal
...
class MockPositionSizer:
    ...
    async def calculate_position_size(self, signal, current_equity, current_positions):
        ...
        logger.info(
            f"Position sizing: equity=${current_equity:.2f}, max_exposure=${max_exposure:.2f}, price={estimated_price}, units={position_size}"
        )
...
class MockRiskManager:
    ...
    async def check_pre_trade_risk(self, order, current_positions, current_equity):
        ...
        total_exposure = sum(
            abs(pos.quantity * pos.avg_price) for pos in current_positions.values()
        )
        ...
        # Debug logging
        logger.info(
            f"Risk check: current_exposure=${total_exposure:.2f}, new_trade=${new_exposure:.2f}, total=${total_after_trade:.2f}, max_allowed=${max_allowed:.2f}"
        )
...
class MockMarketReplay:
    ...
    async def load_data(self):
        ...
        logger.info(
            f"MockMarketReplay: Generating data for {self.symbols} from {self.start_date} to {self.end_date}"
        )
        ...
        prices = {
            symbol: 1.1000 + np.random.uniform(-0.01, 0.01) for symbol in self.symbols
        }
        ...
        while current_time <= end_time:
            for symbol in self.symbols:
                ...
                data_point = DataPoint(
                    symbol=symbol, timestamp=current_time, data=ohlcv, data_type="bar"
                )
    ...
    async def stream(self):
        ...
        logger.info(
            f"MockMarketReplay: Starting to stream {len(self.data_points)} data points"
        )
        for i, data_point in enumerate(self.data_points):
            if i % 20 == 0:
                logger.info(
                    f"MockMarketReplay: Streaming data point {i+1}/{len(self.data_points)}"
                )
            yield data_point
