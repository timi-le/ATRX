...
# --- Import Core Modules ---
from core.ml_predictor import XGBoostPredictor, LSTMPredictor, CNNPredictor
...
async def main():
    ...
    for tf_str, df_list in all_symbols_mtf_data.items():
        if df_list:
            df = pd.concat(df_list).sort_values('time').drop_duplicates('time').set_index('time')
    ...
    feature_df = await feature_engine.compute_mtf_features(
        mtf_data=combined_mtf_data,
        primary_tf=PRIMARY_TIMEFRAME
    )
    
    print(f"Generated {len(feature_df.columns)} features for {len(feature_df)} data points.")
    if feature_df.empty:
    ...
    if primary_tf_atr_col not in feature_df.columns:
        ...
        feature_df[primary_tf_atr_col] = _compute_atr(feature_df['high'].values, feature_df['low'].values, feature_df['close'].values, period=14)

    labeling_input_df = feature_df[['close', primary_tf_atr_col]].rename(columns={primary_tf_atr_col: 'atr'})
    ...
    final_features = final_df.drop(columns=['open', 'high', 'low', 'close', 'tick_volume', 'spread', 'real_volume', 'triple_barrier_label'])
    final_labels = final_df['triple_barrier_label']
    
    print(f"Generated {len(final_labels)} labels. Distribution:\n{final_labels.value_counts(normalize=True)}")
    
    if final_features.empty or final_labels.empty:
        print("FATAL: No data remaining after feature generation and labeling. Cannot train.")
        return
    ...
    # CNN
    print("\n--- Training CNN Model ---")
    cnn_predictor = CNNPredictor(n_features=final_features.shape[1], image_size=int(np.sqrt(final_features.shape[1])))
